...
1. Close and exercise the sequential register-bank endpoint. The pure-open
   ...
   registered consumers are qualified. A two-bit posted-storage footprint
   passes all four values, immediate write/read, and back-to-back newest-write
   ...
   the data boundary lane-by-lane from the qualified two-bit posted footprint.
