...
if find "$build_root" -mindepth 1 -print -quit | grep -q .; then
	echo "Phase1B build root must be empty: $build_root" >&2
	exit 2
fi

case "${MSYSTEM:-}" in
	UCRT64) ;;
	*) echo "Phase1B requires the frozen MSYS2 UCRT64 environment" >&2; exit 2 ;;
esac
...
python_script=$(cygpath -w "$script_dir/phase1b.py")
...
common_cflags=(
	-O2
	-g0
	"-ffile-prefix-map=$build_root=/usr/src/r6-phase1b"
	"-fdebug-prefix-map=$build_root=/usr/src/r6-phase1b"
)
...
(
	...
	export LIBUSB1_LIBS="-L$libusb_prefix/lib -lusb-1.0 -ladvapi32 -lsetupapi -lole32 -luser32"
...
echo "PASS_PHASE1B_BUILD_COMPLETE_OPENOCD_NOT_EXECUTED"
