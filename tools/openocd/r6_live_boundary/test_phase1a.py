...
import tools.openocd.r6_live_boundary.phase1a as phase1a
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.__setitem__("required_future_gates", ["PLACEHOLDER"]),
        lambda value: value["toolchain_decision"].__setitem__(
            "distribution", "UNTRUSTED"
        ),
        lambda value: value["jimtcl"].__setitem__(
            "required_absent_final_objects", []
        ),
        lambda value: value["libusb"].__setitem__("forbidden_backend_objects", []),
        lambda value: value["libusb"].__setitem__("forbidden_runtime_libraries", []),
        lambda value: value["libusb"].__setitem__(
            "required_system_library_resolution", "RELATIVE_SEARCH"
        ),
        lambda value: value["earliest_main_gate"].__setitem__("required_order", []),
    ],
)
def test_manifest_rejects_semantic_contract_weakening(mutate) -> None:
    value = manifest()
    mutate(value)
    with pytest.raises(Phase1AFailure, match="semantic identity"):
        validate_manifest(value)


def test_candidate_identity_rejects_parent_merge_path_drift_and_dirty_tree(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    parent = phase1a.ACCEPTED_PHASE0_V11

    monkeypatch.setattr(
        phase1a,
        "git",
        lambda _repo, *_args: f"{parent} parent-of-v11",
    )
    with pytest.raises(Phase1AFailure, match="not an exact child"):
        phase1a.validate_candidate_parent(Path("unused"), parent)

    monkeypatch.setattr(
        phase1a,
        "git",
        lambda _repo, *_args: f"candidate {parent} second-parent",
    )
    with pytest.raises(Phase1AFailure, match="exactly one parent"):
        phase1a.validate_candidate_parent(Path("unused"), parent)

    def path_drift_git(_repo: Path, *args: str) -> str:
        if args[0] == "rev-list":
            return f"candidate {parent}"
        if args[0] == "diff":
            return "tools/openocd/r6_live_boundary/phase1a.py"
        return ""

    monkeypatch.setattr(phase1a, "git", path_drift_git)
    with pytest.raises(Phase1AFailure, match="path inventory differs"):
        phase1a.validate_candidate_parent(Path("unused"), parent)

    def dirty_git(_repo: Path, *args: str) -> str:
        if args[0] == "rev-list":
            return f"candidate {parent}"
        if args[0] == "diff":
            return "\n".join(sorted(phase1a.EXPECTED_CHANGED_PATHS))
        return " M tools/openocd/r6_live_boundary/phase1a.py"

    monkeypatch.setattr(phase1a, "git", dirty_git)
    with pytest.raises(Phase1AFailure, match="worktree is not clean"):
        phase1a.validate_candidate_parent(Path("unused"), parent)
