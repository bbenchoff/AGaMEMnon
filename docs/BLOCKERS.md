...
|---|---|---|
| Final compiled and source regression | Prior candidate full CI `34089341861` passed all eight jobs. Default odd-source images reproduce exactly; three native mode tests pass; two obsolete native even-only assertions were corrected and pass | Current CI `34095135068` has 268 native tests passing and two failures in obsolete even-only output-bridge assertions; installed-wheel jobs pass while the full source suites remain running. Correct the assertions to verify actual admission and rerun the native gate |
...
| Worktree and branch consolidation | Twenty-eight worktrees have been retired: 102 originally, 74 remain. Twenty-nine local branches have been retired: 74 to 45. Five redundant remote branches were also removed. Recovery tips, bundles and checkout archives are banked | Preserve remaining unique commits, local changes, ignored files and nested repositories before deleting redundant checkouts and branches |
