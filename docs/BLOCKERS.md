...
| Installation and release artifacts | Isolated wheel repacking passed; missing routing datasets packaged. Versioning and tag-bound notes integrated | Installed first BRAM source profile reaches emission but final hash differs: only its ground route differs from the qualified reference (20 removed / 14 added hops). Preserve and reserve the qualified constant tree, then rerun all four profiles and complete programming/archive gates |
