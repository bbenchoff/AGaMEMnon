...
| Programming | Recent bounded SRAM session passes with research instrumentation. Fresh CI-wheel installation succeeds, but anonymous `install-openocd` returns HTTP 404: the requested `openocd-v0.1.0` tag/release is absent | Publish the reproducible OpenOCD dependency after build and validation. Fix 5f9931f preserves pristine packaging source and normalizes MSYS Git paths; 38 local bundle tests pass. Fresh four-platform build 34086984598 is pending. Installed programming remains unqualified |
