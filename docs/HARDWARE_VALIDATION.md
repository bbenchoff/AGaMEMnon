...
| Hard UART0 external TX | TX only on physical L48 PIN_10 through the exact open peripheral-route fabric. The old wrong-clock run produced ~560 baud when 9600 was requested. Using `ag32_uart_ref_hz_measured()`, the independent Pico PIO UART receiver decoded 64/64 exact `FF 55 41 00` pattern bytes at each requested 9600, 38400, and 115200 baud. RX, flow control, UART1–4, another clock state/board, and sub-percent absolute accuracy remain unqualified |
| Hard I2C0 master write/read | Active open-drain single-byte interoperability on physical L48 pads (SDA PIN_11, SCL PIN_15): an RP2350 slave at `0x55` supplied pull-ups, ACKed both address directions and write byte `0xA6`, and returned read byte `0x5A`; AG32 terminated with NACK+STOP. The repaired last-byte SDK path accepts the controller's expected terminal `SR.RXNACK`. The earlier 288-transaction no-slave framing/NACK capture remains qualified. Multi-byte transfers, repeated START, clock stretching, 10-bit addressing, arbitration, I2C1, and absolute SCL timing are NOT qualified |
