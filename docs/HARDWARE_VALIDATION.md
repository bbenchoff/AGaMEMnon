...
| External AHB write | The complete public L48 profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled wait, exact zero-extended reads, aligned byte/halfword semantics and fail-closed non-SINGLE bursts. A separate exact 16-bit held-scratch checkpoint now passes aligned word writes, independent byte +0/+1 writes, rejects byte +2/+3 and aligned writes +4/+8/+c, and clears on GPIO reset. Four complete 100-pattern word/byte runs had zero mutation, overwrite, isolation or reset errors; the prior retention control remained green. Foreign reads still alias +0. It uses sixteen external-feedback state cells and zero direct-D cells. Only HRDATA[15:0] is claimed there; halfword access, read/full address decoding, upper lanes, bursts, arbitrary widths/placement and public-bank integration remain unsupported |
| External AHB request controls | Registered `HADDR[4:2]` capture through `MCU_DIN76:78`; eight values observed 32 times each over 256 reads. An isolated pure-open `HADDR[5:4]` XOR additionally passed 256/256 addresses; simultaneous `HADDR[1:0]` logic ingress is qualified on the complete-byte waited bank. The 16-bit checkpoint composes HADDR[3:2], HADDR0 and HSIZE1 for bounded word/byte write selection; its reads are not decoded. HSIZE0 and halfword storage semantics remain open |
