...
| External AHB write | The complete public L48 profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled wait, exact zero-extended reads, aligned byte/halfword semantics and fail-closed non-SINGLE bursts. A separate exact 16-bit held-scratch checkpoint now passes aligned word and halfword +0 writes, independent byte +0/+1 writes, rejects byte +2/+3, aligned halfwords +2/+4/+8/+c, and aligned words +4/+8/+c, and clears on GPIO reset. Four complete 100-pattern word/byte/halfword runs had zero mutation, overwrite, isolation or reset errors; HSIZE0 identity/zero and retained word/byte controls remained green. Foreign reads still alias +0. It uses sixteen external-feedback state cells and zero direct-D cells. Only HRDATA[15:0] is claimed there; misaligned transfers, read/full address decoding, upper lanes, bursts, arbitrary widths/placement and public-bank integration remain unsupported |
