...
| External AHB write | All 32 HWDATA lanes exercised in protocol-valid four-bit groups; HWDATA0 through HWDATA5 additionally have exact one-consumer MCU_BUS_CLOCK capture footprints, direct HWDATA0 at lane-zero storage is live, and a pure-open six-bit HADDR2-tagged posted store passes all 64 values, immediate/back-to-back readback, persistence, and offset isolation. Lane5 uses a qualified HWDATA5 capture, separate commit/hold next-state LUT, and ordinary one-input storage DFF; two characterized route-through leaves distribute commit. Lanes6..7, integrated reset, waits/errors, and byte/halfword behavior remain open |
