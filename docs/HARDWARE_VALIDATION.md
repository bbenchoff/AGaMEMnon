...
| External AHB request controls | Registered `HADDR[4:2]` capture through `MCU_DIN76:78`; eight values observed 32 times each over 256 reads. An isolated pure-open `HADDR[5:4]` XOR additionally passed 256/256 addresses; simultaneous `HADDR[1:0]` logic ingress is qualified on the complete-byte waited bank. The exact 16-bit checkpoint composes HADDR[3:2], HADDR[1:0], HSIZE[1:0], HTRANS and HWRITE for bounded aligned word/halfword and byte selection; its reads are not decoded and misaligned transfers are outside scope |
