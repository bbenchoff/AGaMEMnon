...
| External AHB write | The complete public L48 profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled wait, exact zero-extended reads, aligned byte/halfword semantics and fail-closed non-SINGLE bursts. A separate exact 16-bit held-scratch checkpoint passes 100 aligned word patterns with zero immediate, SRAM-churn-retention or repeated-read errors and clears on GPIO reset. A retained derivative proves that aligned writes to +4/+8/+c cannot alter +0; foreign reads still alias +0. It uses sixteen external-feedback state cells and zero direct-D cells. Only HRDATA[15:0] is claimed there; read/full address decoding, subword access, upper lanes, bursts, arbitrary widths/placement and public-bank integration remain unsupported |
| External AHB request controls | Registered `HADDR[4:2]` capture through `MCU_DIN76:78`; eight values observed 32 times each over 256 reads. An isolated pure-open `HADDR[5:4]` XOR additionally passed 256/256 addresses; simultaneous `HADDR[1:0]` logic ingress is qualified on the complete-byte waited bank. The 16-bit checkpoint consumes HADDR[3:2] for bounded write-side +0 isolation only; its reads are not decoded. One exact HSIZE[1] corridor returns one for 256 word reads and zero for 256 halfword plus 256 byte reads at a fixed address. That qualifies the control ingress, not subword storage semantics or arbitrary placement |
