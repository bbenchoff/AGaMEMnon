...
| Hard UART0 external TX/RX | Separate exact L48 routes qualify TX on PIN_10 and RX on PIN_31. For TX, an independent Pico PIO receiver decoded 64/64 exact `FF 55 41 00` bytes at requested 9600, 38400, and 115200 baud. For RX, the reference-board DAP CDC transmitter sent the same pattern; three fresh full-load runs received 64/64 bytes at all three rates with zero HAL error. The old wrong-clock TX run produced ~560 baud when 9600 was requested. Simultaneous duplex, flow control, FIFO/error injection, UART1–4, another clock state/board, the ROM protocol, and sub-percent absolute accuracy remain unqualified |
