...
| External AHB write/read checkpoint | The default exact L48 profile composes immutable ID8 `0x4d` at +0, held scratch16 at +4, counter3 at +8, and W1C1 at +c. Four sequential SRAM-only runs (three deterministic qualification runs plus a final production-runner control) all returned FCB `0x000f0002`, eight zero error groups, counter coverage `0xff`, and reset-final low-16 words `[0x004d,0,0,0]`. The matrix covers aligned word/halfword and independent byte +4/+5 scratch writes, representative foreign-address rejection, decoded reads, retention, coexistence, GPIO reset, counter range/liveness, and qualification-hook W1C set/clear with set priority. The composer retains the 101-cell +4 checkpoint, mutates only six reviewed overlay LUT/input selections, extends ten routes append-only, and adds 18 exact cells; strict pack has zero legacy, predicted, or unmapped selectors. Raw HRDATA[31:16], canonical 32-bit identity, production status-set ingress, misaligned and signed loads, higher/full-window decode, bursts on this composition, arbitrary widths/placement, other packages, and a generic 32-bit ABI remain unsupported. The older complete-byte profile and its exact-zero/read/burst evidence remain retained separately |
