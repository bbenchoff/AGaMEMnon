...
| Four left-edge OE corridors | Exact L48 PIN_25 through PIN_28 corridors only. The retained vendor-routed quad oracle configured at FCB `0x000f0002`; under an external pull-up, all four links repeatedly alternated release-high and drive-low according to their independent XOR enables. PIN_28 inverted on every shared-common transition even though its second XOR input (PIN_20) is not instrumented, which removes that input's unknown polarity from the proof. Under pull-down, every asserted state remained low; released open-drain lines are allowed to read low. This qualifies active-high OE polarity and the four exact corridor/pad compositions, not ordinary source ingress for PIN_26 through PIN_28, active drive-high, thresholds, or the full node's phase/readback network. Evidence: `qualification/bidir_left_quad_evidence.jsonl` |
...
| Physical output | Characterized L48 header outputs and PIN_25 through PIN_28, including concurrent use; plus **all ten top-edge decimal physical package leads PIN_10 through PIN_19** (2026-08-15). `PIN_n` is the decimal L48 package-lead label, not a hexadecimal index. The closing PIN_10 and PIN_11 single-pad images toggle only GP4 and GP1 respectively under both Pico pulls; their same-tile pair toggles both simultaneously while every other observed lead remains static. All three production images build from the ordinary PCF flow without a route blacklist or vendor-slice shell override, with zero unmapped, predicted or legacy selectors and FCB 0x000f0002; the retained pair repacks byte-identically. The default config tile's stale selector fields are replaced, not ORed. The claim remains exact L48 output compositions, not arbitrary routes, bidirectionality, electrical modes, or other packages. The left-edge four also reproduce from the ordinary CLI. Flow caveat: that ordinary-CLI recipe is the Python-architecture PCF placer, which composes experimental options and needs `--research-unsafe`. `agamemnon build <source> --uarch --pcf <pcf>` (no `--research-unsafe`) also builds release-strict, zero-unmapped/predicted/legacy-selector images for the left-edge four and nine of the ten top-edge leads (all but PIN_15) -- `AGAMEMNON_VENDOR_OUT_SLICE` is now release-admitted for exactly the four qualified presentations, value-gated so nothing else is -- and every one of those images FCB-configured the real device to 0x000f0002 over a non-destructive SRAM session (`io_evidence.jsonl` trial `pad-uarch-pcf-release-strict-vehicle-config-accept-20260817`). The Pico toggle re-witness of that vehicle's own images is closed for all ten (`io_evidence.jsonl` trial `pad-uarch-pcf-toggle-rewitness-20260817`): each of the left-edge four and the nine top-edge outputs toggles under both Pico pulls on exactly its intended lead and no other, with the same weak/coupled-on-one-pull-only pattern on adjacent leads the earlier campaigns already classified as capacitive pickup, not a driven pad. The toggle/electrical re-witness for this vehicle's own five qualified-input demonstration builds (registered input on PIN_10/PIN_11/PIN_19, combinational input-to-PIN_16 on PIN_12/PIN_15) is closed (`io_evidence.jsonl` trial `pad-uarch-input-rewitness-closed-silicon-20260817`): under the corrected active-drive procedure all five correctly track the driven input on both drive levels, reproducibly across two independent full sweeps; the research-unsafe-vehicle input qualification is unaffected. An initial negative (the observed PIN_16 output read a fixed level regardless of the Pico's applied bias) did not reproduce in a fresh session and is attributed to a session/rig-specific condition rather than a vehicle, procedure, or bitstream defect. PIN_15 as an output still fails to route under `--uarch` and still needs `--research-unsafe` under either vehicle |
...
| BRAM Port A | One characterized X13Y4 x18 open-flow path plus all nine X13Y4 read-only x9 data bits through exact per-lane projections and one simultaneous strict-open 256-word identity bundle; bits3, 4, and 5 independently match word-address bit3, bits6, 7, and 8 match word-address bits0, 1, and 2 respectively, all for 256/256 reads, and an independent HADDR11/AddressA12 projection distinguishes word addresses 0 and 512 for 64/64 alternating samples. Separate zero-LUT vendor-routed images prove distinct simultaneous X13Y1..Y4 marker reads and all 512 x18 Port-A addresses at all four sites with zero first-read, settled-read, and upper-half errors. Those multi-site results qualify the arrays and exact vendor compositions, not open-flow routing outside X13Y4. **Writable (2026-09-14/15, experimental site-read profile):** the MCU wrote real BRAM data over External-AHB and read it back — up to a **five-bit** open-flow write at X13Y4 (`DataInA[0]`/`[2]`/`[3]`/`[4]`/`[6]`→`DataOutA` same lanes), board-witnessed. Lanes `[0]`/`[2]`/`[4]` are proven genuine array mutation by an INIT=1/write-0 test that drove the direct hard output 1→0 (no read-first path can do that); the two additional lanes `[3]`/`[6]` are proven by a two-word distinct-value readback (word0 reads `0b011`, word1 reads `0b100`) that rules out a global set-only latch. Writable data width is bounded by characterized `DataInA` ingress lanes, not width mode. `ByteEnA` byte-lane masking is functional on silicon, and its config family is now recovered: per-byte write-enable is a **`CFG_KMUX` local-gnd tie** at the BRAM tile (position 8 of each nine-selector lane), *not* a routed net — which is why the earlier open-flow attempt that routed a LUT constant saw ByteEn=10 and =01 write identically (an AGaMEMnon routing/delivery gap, not a silicon limit; that "refuted" reading is withdrawn). Both bits are board-proven: `ByteEnA[1]`→gnd via `CFG_KMUX` sel26 (vendor x18/INIT=1/ByteEnA=01 image held the high byte at INIT-1 while the low byte tracked the write), and `ByteEnA[0]`→gnd via `CFG_KMUX` sel17 proven on AGaMEMnon's *own* open-flow image (a single-config-bit A/B: asserting sel17 masked the low byte, flipping obs `0xE4`→`0xFF` with reads stuck at INIT-1). AGaMEMnon now **natively emits** this per-byte mask (opt-in `AGAMEMNON_BRAM_BYTEEN`): a constant-0 Port-A `ByteEnA` lane in the design is carried from the packer (`qin_pack` stamps `AGM_BYTEEN_A_MASK`) to bitgen, which emits the `CFG_KMUX` pos-8 gnd tie. It is **board-proven natively (2026-09-15)**: a natively-built A/B (`image_w356` vs `image_w356_be10`, differing by exactly the one emitted config byte + CRC) masked the low-byte write on silicon (obs `0x23`→`0x00`, cells held at INIT); default builds are byte-identical (the path fires only on a constant-0 lane). Correct native narrow-*width* (x1/x2/x4/x9) sub-word writes additionally need the packer's DataIn sub-word window; narrow-*width* memories ≤512 words are already writable by x18 mapping. These writable results are qualified observations at X13Y4, not a fresh-route write-mode qualification. |
...
| Hard SPI routes | SPI0 and SPI1 TX pass exact L48 mode-3/MSB-first/active-low-CS, 1–4-byte, documented-divider, and raw TX-byte-order contracts. One older retained SPI0 IO1 image qualifies active receive only for that immutable composition. Corrected typed SPI0/SPI1 MISO images now pass the [bounded receive contract](SPI_RECEIVE_QUALIFICATION.md), with vendor positives and old-image stuck-high negatives. No generic RX/duplex, dual/quad, DMA/POLL/interrupt, other modes/routes/packages, or absolute SCK claim |
...
> The device retains no isolated dead-edge classifications. The 14 edges once
> framed that way came from a single large, *congested* MCU-exit design and were
> mis-attributed to individual edges: **all 14 are board-proven to conduct** in
> clean/isolated builds and are un-gated in the shipped router, leaving no
> conservatively blocked edge. Forcing constructions' STUCK readings remain
> uninterpretable: a matched sibling control keeping a different non-catalogued
> crossing also read STUCK, so only positive readings count. The evidence is
