...
| External AHB write/read checkpoint | The complete public L48 profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled wait, exact zero-extended reads, aligned byte/halfword semantics and fail-closed non-SINGLE bursts. A separate exact 16-bit held-scratch checkpoint passes aligned word and halfword +0 writes, independent byte +0/+1 writes, rejects byte +2/+3, aligned halfwords +2/+4/+8/+c, and aligned words +4/+8/+c, clears on GPIO reset, and returns low-16 aligned word reads at +0/+4/+8/+c as `[state,0,0,0]`. Four complete 100-pattern write runs and ten sequential causal/read-decoder runs had zero relevant errors. Three repeated 32-pattern/128-observation runs additionally qualify CPU-visible aligned unsigned `LBU +0/+1` and `LHU +0` lane selection and zero extension. A two-INIT derivative rebases the same object to +4 and passes three 32-pattern/160-observation runs covering accepted word/halfword/byte writes, foreign-offset rejection, decoded word/subword reads, retention and reset. It uses sixteen external-feedback state cells and zero direct-D cells. The +4 object does not yet coexist with the public ID/counter/W1C objects. Misaligned and signed loads, the raw HRDATA[31:16] value, higher/full-window decode, bursts, arbitrary widths/placement and full public-bank integration remain unsupported |
