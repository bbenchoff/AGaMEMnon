...
| External AHB write/read checkpoint | The default exact L48 profile composes canonical ID32 `0x4147414d` at +0, zero-extended scratch16 at +4, counter3 at +8, and W1C1 at +c. Three sequential SRAM-only full-map runs returned FCB `0x000f0002`, nine zero error groups, counter coverage `0xff`, eight scratch observations, and reset-final exact words `[0x4147414d,0,0,0]`. The matrix covers unmasked LW values, every ID byte/halfword lane, unsigned-load zero extension, scratch word/halfword and independent-byte writes, foreign-address rejection, retention, coexistence, GPIO reset, counter range/liveness, and qualification-hook W1C set/clear/set-priority. The composer retains the 119-cell public16 checkpoint, mutates only read_gate8/read_gate14, adds 16 exact HRDATA exits plus one ID selector, and relocates one status-pending branch while preserving all three consumers; strict pack has zero legacy, predicted, or unmapped selectors. Production status-set ingress, misaligned and signed loads, higher/full-window decode, bursts, arbitrary widths/placement, other packages, and a generic register-bank generator remain unsupported. The public16 and older complete-byte profiles remain retained separately |
