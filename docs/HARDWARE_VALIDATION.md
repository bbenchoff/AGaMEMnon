...
| External AHB write | All 32 HWDATA lanes are exercised in protocol-valid four-bit groups, and a pure-open complete-byte HADDR2-tagged posted store passes all 256 values, immediate/back-to-back readback, persistence, and offset isolation through exact HWDATA0..7 consumer/storage footprints. Lane5 uses a qualified HWDATA5 capture, separate commit/hold next-state LUT, and ordinary one-input storage DFF; two characterized route-through leaves distribute commit. Lane6 folds commit/hold into its exact X14Y12 slice15 HWDATA6 consumer while HREADYOUT uses a separate constant-high corridor. Lane7 folds HWDATA7, commit, and own-Q hold into the exact X14Y11 slice0/I1 consumer. Register classes, integrated reset, waits/errors, and byte/halfword transfer semantics remain open |
