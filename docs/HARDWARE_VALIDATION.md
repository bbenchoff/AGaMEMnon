...
| BRAM Port A | One characterized x18 path plus X13Y4 read-only x9 with visible data bits 0..5; bits3, 4, and 5 independently match word-address bit3 for 256/256 reads, and an independent HADDR11/AddressA12 projection distinguishes word addresses 0 and 512 for 64/64 alternating samples |
...
- The BRAM result does not qualify every tile, width, initialization layout,
  ...
  qualified for visible data bits 0..5 within the exact exercised projections.
  ...
  reads. Bit5 uses the corrected exact BufMUX13/RMUX92/RMUX75/RMUX20/BBMUXE07
  ...
  6..8, the remaining high-address lanes/range, writes, output registers,
