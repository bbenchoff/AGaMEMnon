...
| Physical input | L48 PIN_10, PIN_11, PIN_15, and PIN_19; registered input on PIN_19. PIN_25 through PIN_28 are also qualified through their four exact left-edge corridors in single-consumer direct inversions observed at PIN_18; this does not qualify general fanout or the complete four-link bidirectional node |
...
| External AHB write/read checkpoint | The complete public L48 profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled wait, exact zero-extended reads, aligned byte/halfword semantics and fail-closed non-SINGLE bursts. A separate exact 16-bit held-scratch checkpoint passes aligned word and halfword +0 writes, independent byte +0/+1 writes, rejects byte +2/+3, aligned halfwords +2/+4/+8/+c, and aligned words +4/+8/+c, clears on GPIO reset, and returns low-16 aligned word reads at +0/+4/+8/+c as `[state,0,0,0]`. Four complete 100-pattern write runs and ten sequential causal/read-decoder runs had zero relevant errors. It uses sixteen external-feedback state cells and zero direct-D cells. Only HRDATA[15:0] aligned word reads are claimed there; subword read-lane semantics, misaligned transfers, higher/full-window decode, upper lanes, bursts, arbitrary widths/placement and public-bank integration remain unsupported |
| External AHB request controls | Registered `HADDR[4:2]` capture through `MCU_DIN76:78`; eight values observed 32 times each over 256 reads. An isolated pure-open `HADDR[5:4]` XOR additionally passed 256/256 addresses; simultaneous `HADDR[1:0]` logic ingress is qualified on the complete-byte waited bank. The exact 16-bit checkpoint composes HADDR[3:2], HADDR[1:0], HSIZE[1:0], HTRANS and HWRITE for bounded aligned word/halfword and byte write selection, and HADDR[3:2] for low-16 aligned word-read isolation. Subword read-lane semantics and misaligned transfers remain outside scope |
