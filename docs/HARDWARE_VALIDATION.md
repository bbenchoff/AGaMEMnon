...
| BRAM Port A | One characterized X13Y4 x18 open-flow path plus all nine X13Y4 read-only x9 data bits through exact per-lane projections and one simultaneous strict-open 256-word identity bundle; bits3, 4, and 5 independently match word-address bit3, bits6, 7, and 8 match word-address bits0, 1, and 2 respectively, all for 256/256 reads, and an independent HADDR11/AddressA12 projection distinguishes word addresses 0 and 512 for 64/64 alternating samples. Separate zero-LUT vendor-routed images prove distinct simultaneous X13Y1..Y4 marker reads and all 512 x18 Port-A addresses at all four sites with zero first-read, settled-read, and upper-half errors. Those multi-site results qualify the arrays and exact vendor compositions, not open-flow routing outside X13Y4. **Writable (2026-09-14/15, experimental site-read profile):** the MCU wrote real BRAM data over External-AHB and read it back — up to a **three-bit** open-flow write at X13Y4 (`DataInA[0]`/`[2]`/`[4]`→`DataOutA[0]`/`[2]`/`[4]`), proven to be genuine array mutation by an INIT=1/write-0 test that drove the direct hard output 1→0 (no read-first path can do that). Writable data width is bounded by characterized `DataInA` ingress lanes, not width mode. `ByteEnA` byte-lane masking is functional on silicon (a vendor-flow x18/INIT=1/ByteEnA=01 image held the high byte at INIT-1 across all words while the low byte tracked the write); an earlier open-flow attempt that saw ByteEn=10 and =01 write identically was an AGaMEMnon routing/delivery gap, not a silicon limit, and that "refuted" reading is withdrawn. So native narrow-mode packing writes are achievable once AGaMEMnon replicates the vendor's conducting ByteEn routing; narrow-*width* memories ≤512 words are already writable by x18 mapping. |
