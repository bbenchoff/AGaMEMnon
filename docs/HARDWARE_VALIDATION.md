...
| External AHB write | All 32 HWDATA lanes exercised in protocol-valid four-bit groups; HWDATA0 through HWDATA5 additionally have exact one-consumer MCU_BUS_CLOCK capture footprints, direct HWDATA0 at lane-zero storage is live, and a pure-open five-bit HADDR2-tagged posted store passes all 32 values, immediate/back-to-back readback, persistence, and offset isolation. X14Y11 slice5 DD88 storage follows HWDATA5 with commit tied high, but routed-control six-bit variants retain exact bits 0..4 while lane5 remains constant one, so six-bit storage is not claimed |
