...
| External AHB write | All 32 HWDATA lanes exercised in protocol-valid four-bit groups; HWDATA0 through HWDATA6 additionally have exact one-consumer MCU_BUS_CLOCK capture/storage footprints, direct HWDATA0 at lane-zero storage is live, and a pure-open seven-bit HADDR2-tagged posted store passes all 128 values, immediate/back-to-back readback, persistence, and offset isolation. Lane5 uses a qualified HWDATA5 capture, separate commit/hold next-state LUT, and ordinary one-input storage DFF; two characterized route-through leaves distribute commit. Lane6 folds commit/hold into its exact X14Y12 slice15 HWDATA6 consumer while HREADYOUT uses a separate constant-high corridor. Lane7, integrated reset, waits/errors, and byte/halfword behavior remain open |
