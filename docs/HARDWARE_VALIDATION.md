...
| External AHB write | All 32 HWDATA lanes are exercised in protocol-valid four-bit groups, and a pure-open complete-byte HADDR2-tagged posted store passes all 256 values, immediate/back-to-back readback, persistence, and offset isolation through exact HWDATA0..7 consumer/storage footprints. Immutable ID low byte `0x4d` at offset 0 and writable scratch at offset 4 are qualified, including ignored ID writes. A standalone read-only three-bit counter at offset 8 covers all eight states at a constant modulo-eight delta, ignores writes, and isolates other offsets. A standalone one-bit W1C status at offset C passes software-set, repeated hold, zero-write no-op, clear, wrong-offset isolation, re-arm, and set priority. Lane5 uses a qualified capture/next-state/storage chain; lanes6 and 7 fold data, commit, and own-Q hold into exact consumers. Combined full-bank integration, integrated reset, waits/errors, and byte/halfword transfer semantics remain open |
