...
| Hard SPI0 master transmit + RX lane | MOSI/SCK/CSN on physical L48 pads (SCK PIN_12, MOSI PIN_14, CSN PIN_13): 233/233 decoded words all `0x55`, plus a `11 22 33 44` payload with 108 pattern matches. **MSB-first, and word boundaries require CS.** A second exact route adds IO1 on PIN_17: sampled-high RX widths 1–4 returned raw `A50000FF`, `A500FFFF`, `A5FFFFFF`, `FFFFFFFF`, proving valid RX bytes occupy the low-order lanes and stale upper state must be masked. Arbitrary slave-driven values, byte order within multi-byte RX, full duplex interoperability, DUAL/QUAD, and DMA remain unqualified |
