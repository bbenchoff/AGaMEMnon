...
| External AHB write | All 32 HWDATA lanes are exercised in protocol-valid four-bit groups. A posted pure-open bank integrates ID `0x4d`, complete-byte scratch, a three-bit read-only counter, and one-bit W1C status at offsets 0/4/8/C, including GPIO4.1 synchronous reset. A separate current-public image qualifies one controlled write wait with scratch lanes 0–5 and 7 while lane 6 fails closed to zero; exact zero-extended halfword reads are qualified, but exact 32-bit reads and byte semantics are not in current public main. A later complete-byte/32-bit workbench result awaits public integration. Hard MCU_RESETN and deterministic MCU fault behavior from HRESP remain unsupported |
