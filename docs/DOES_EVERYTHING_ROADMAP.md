...
BRAM is a bounded X13Y4 *read* proof plus 39 experimental config rows; the
behaviour matrix (writes, byte enables, output registers, width/mode, independent
clocks, collision/read-during-write, high addresses, other sites) is the labor,
and it is exactly what the HIL instrument makes cheap. Carry needs arbitrary
