...
| 6 | Peripheral plane — hard MMIO breadth | DECODE + SILICON + BENCH | 9 blocks silicon-qualified (incl. UART0 pad TX, I2C0 framing, SPI0 TX plus active 1–4-byte TX-then-RX); UART/I2C receive, simultaneous SPI full-duplex, bit rates, SPI1/I2C1/UART1-4 open; CAN has register activity but **no bits on a wire** | typed drivers + non-destructive evidence per block; use active I2C/UART slaves for receive interoperability; CAN/Ethernet/USB-host need a transceiver/PHY/host (BENCH); RTC/IWDG need an LSI/LSE clock (BENCH) |
