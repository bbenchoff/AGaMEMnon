...
| 7 | MCU External-AHB slave breadth | DECODE -> SILICON | complete-byte public bank; exact 16-bit held-scratch checkpoint with aligned word/halfword +0, independent byte +0/+1, byte/halfword/word foreign-write rejection, low-16 aligned word reads `[state,0,0,0]` at +0/+4/+8/+c, and CPU-visible aligned unsigned byte/halfword lane selection and zero extension; 32-bit reads on the byte bank; aligned byte/half on both exact write profiles; SINGLE only | integrate the 16-bit checkpoint into the public bank; raw upper-HRDATA characterization; higher/full-window decode and upper-lane zeros; misaligned and signed loads; hard `MCU_RESETN`; alternate/PLL3 bus clocks; generic direct-D lowering; full protocol modes |
