...
| 6 | Peripheral plane — hard MMIO breadth | DECODE + SILICON + BENCH | 9 blocks silicon-qualified (incl. UART0 pad TX, active I2C0 single-byte write/read, SPI0 TX plus active 1–4-byte TX-then-RX); UART receive, multi-byte/repeated-START I2C, simultaneous SPI full-duplex, absolute bit rates, SPI1/I2C1/UART1-4 open; CAN has register activity but **no bits on a wire** | typed drivers + non-destructive evidence per block; extend the active I2C oracle and add an active UART source; CAN/Ethernet/USB-host need a transceiver/PHY/host (BENCH); RTC/IWDG need an LSI/LSE clock (BENCH) |
