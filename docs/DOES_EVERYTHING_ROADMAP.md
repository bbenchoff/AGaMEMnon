...
| 7 | MCU External-AHB slave breadth | DECODE -> SILICON | complete-byte public bank; exact 16-bit held-scratch checkpoint with aligned word/halfword +0, independent byte +0/+1, and byte/halfword/word foreign-write rejection; 32-bit reads on the byte bank; aligned byte/half on both exact profiles; SINGLE only | 16-bit read/full address decode and public-bank integration; misaligned transfers; hard `MCU_RESETN`; alternate/PLL3 bus clocks; generic direct-D lowering; full protocol modes |
...
repeated reads, and GPIO reset; foreign reads still alias +0. Open: integrating
that width with read/full address decoding, upper-lane zeros and the public
ID/counter/W1C map; misaligned transfers; hard `MCU_RESETN`,
