...
| 6 | Peripheral plane — hard MMIO breadth | DECODE + SILICON + BENCH | 9 blocks silicon-qualified (incl. UART0 pad TX, active I2C0 write/repeated-START/read, SPI0 TX plus active 1–4-byte TX-then-RX); UART receive, I2C stretching/arbitrary lengths, simultaneous SPI full-duplex, absolute bit rates, SPI1/I2C1/UART1-4 open; CAN has register activity but **no bits on a wire** | typed drivers + non-destructive evidence per block; add active UART source and I2C clock-stretch/negative controls; CAN/Ethernet/USB-host need a transceiver/PHY/host (BENCH); RTC/IWDG need an LSI/LSE clock (BENCH) |
...
| 9 | BRAM modes / sites | DECODE -> SILICON | X13Y4 open-flow read subset (x18 A, x2 B, x9 bundle, 1024-word addr); four exact hash-bound x18 profiles qualify one fixed-address registered-source write A/B; exact structural BEL/frame metadata now covers X13Y1..Y4 and one zero-LUT vendor oracle reads distinct marker bytes from all four arrays simultaneously; 39 config rows experimental; PORTA_OUTREG and bounded x2 PORTB_OUTREG each measured at exactly one read clock; PACKEDMODE has a first-order effect with mechanism unknown; CLKMODE is a bounded null across three compositions | qualify and admit open-flow routes for X13Y1..Y3; address-coded multi-site read; ordinary/generic writes, broader dual-port operation, other-mode output-register behaviour, byte enables, width/mode composition, independent clocks, collision/RDW, most B4 rows |
