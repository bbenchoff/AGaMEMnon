...
| 6 | Peripheral plane — hard MMIO breadth | DECODE + SILICON + BENCH | 9 blocks silicon-qualified (incl. separate UART0 pad TX/RX at 9600/38400/115200, active I2C0 write/repeated-START/read, SPI0 TX plus active 1–4-byte TX-then-RX); UART simultaneous duplex/flow/error stress, I2C stretching/arbitrary lengths, simultaneous SPI full-duplex, absolute bit rates, SPI1/I2C1/UART1-4 open; CAN has register activity but **no bits on a wire** | typed drivers + non-destructive evidence per block; add UART framing/parity/error controls and I2C clock-stretch/negative controls; CAN/Ethernet/USB-host need a transceiver/PHY/host (BENCH); RTC/IWDG need an LSI/LSE clock (BENCH) |
