...
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` additionally has measured behaviour — exactly one BRAM clock of added Port-A read latency at X13Y4, in the one exercised read mode | The 39 B4 rows across X13Y1..Y4 are experimental config encodings; `PORTA_OUTREG` (one Port-A read clock, tested read mode) and `PACKEDMODE` have behavioural measurements -- PACKEDMODE was a bounded null in the read-only oracle but has measured first-order effects in the write-path and dual-port oracles, with no mechanism claimed -- while `CLKMODE` remains a bounded null across read, write-path and dual-port. The fabric write did not land, and that is not a silicon verdict. A strict three-image follow-up forced the retained SERV WeA/DataInA/AddressA ingress and still returned the no-write set in three runs, refuting `TMUX13`/`bram_dual_ctrl` source selection as the deciding explanation; exact replay of the retained dependent `serv_multii` store proof is now the next discriminator. General writes, byte enables as behavior, dual-port operation, output-register behaviour on Port B or in other modes, width/mode combinations, independent clocks, collisions/read-during-write, high-address breadth, and other-site behavior remain unqualified. Note also that the X13Y1..Y4 scope is the CONFIG surface (`agamemnon/engine/pips_bram_pll.csv`); the PLACEMENT surface is X13Y4 ONLY |
