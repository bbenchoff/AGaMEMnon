...
| MCU External AHB | Public main has full HRDATA route recovery, a constant slave, a complete-byte bank with one controlled write wait and zero-wait reads, bounded address/control paths, and one-hot interrupt commands | Exact upper-word completion, HADDR[0] byte semantics, hard reset, HRESP errors, bursts, alternate bus clocks, generic direct-D, wider state, complete protocol behavior, AHB master/DMA, and broader peripherals remain |
