...
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` adds exactly one Port-A read clock; bounded x2 `PORTB_OUTREG` adds exactly one Port-B read clock. Four hash-bound, pack-only x18 checkpoints qualify one fixed-address registered-source write A/B through `TMUX09 -> KMUX03` | The 39 B4 rows across X13Y1..Y4 are experimental config encodings. `PACKEDMODE` has first-order effects in bounded write-path-shaped and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null. Direct hard-output controls refute the former wrapper-visible write claim; the later four-arm retained-checkpoint matrix causally shows low retaining INIT and high reaching opposite `DataIn`. Ordinary source-to-route and inferred writes, WeA mechanism/general routing, other addresses, broader dual-port operation, other output-register modes, widths/sites, independent clocks, collisions, patterned INIT, and NEW/NO_CHANGE remain unqualified. Production does not bypass `emulate_read_first` globally. The CONFIG surface spans X13Y1..Y4; PLACEMENT remains X13Y4 only |
...
later registered-source four-arm matrix corrected the apparatus and is exposed
only through hash-bound `agamemnon pack ... --qualified-checkpoint` replay.
That exact replay is qualified; ordinary source-to-route and generic writes are
not.
