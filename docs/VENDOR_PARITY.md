...
| MCU External AHB | Public main has full HRDATA route recovery, a constant slave, a complete-byte bank with one controlled write wait, exact zero-extended 32-bit reads, aligned byte/halfword semantics, fail-closed non-SINGLE burst rejection, bounded address/control paths, and one-hot interrupt commands. A separate exact 16-bit held-scratch checkpoint passes word write/hold/read, SRAM-churn retention, repeated reads, GPIO reset, and write-side isolation of +0 against +4/+8/+c. An exact HSIZE[1] corridor also distinguishes word from halfword/byte transfers | Hard reset, alternate bus clocks, generic direct-D, 16-bit public-bank integration, read/full address decode (foreign reads still alias +0), subword state semantics for the held checkpoint, arbitrary HSIZE placement/composition, complete protocol behavior, AHB master/DMA, and broader peripherals remain; HRESP-to-fault is retired and misaligned CPU accesses are characterized hard-core faults |
