...
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` adds exactly one Port-A read clock; bounded x2 `PORTB_OUTREG` adds exactly one Port-B read clock | The 39 B4 rows across X13Y1..Y4 are experimental config encodings. `PACKEDMODE` has first-order effects in bounded write-path-shaped and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null. Direct hard-output controls refute the former source-built write claim: INIT=1/write-`00` stayed `11`, and INIT=0/write-`11` stayed `00`; the earlier F0/F3 difference was fabric-side wrapper output. A conflict-free x18 matrix then proved Port-A and Port-B clock/address/read liveness and held ClkEn0 plus both byte enables high while changing only WeA low/high; both INIT polarities still retained INIT. Earlier full-control images with cross-net RMUX60/RMUX84 ownership are void, and bitgen now rejects them. Production no longer bypasses `emulate_read_first` automatically. No hard-BRAM write, WeA polarity, or WeA mechanism is qualified. Exact terminal-control semantics, broader dual-port operation, other output-register modes, widths/sites, independent clocks, collisions, patterned INIT, and NEW/NO_CHANGE remain unqualified. The CONFIG surface spans X13Y1..Y4; PLACEMENT remains X13Y4 only |
...
| IO and packages | Selected L48 pins and static input/output behavior; recovered maps for L48/Q32/L64/L100; exact PIN_25 combined-cell constant/dynamic OE and static readback | Only L48 is silicon-qualified. Simultaneous dynamic readback, external-control/generic/registered OE, generic bidirectional behavior, broad electrical attributes, full pinouts, and other packages need hardware qualification |
...
`{2,7}`, clearing inherited sel 3. That is not reset-behavior or write
qualification. The live natural `TMUX13 -> KMUX3` open matrix retained INIT in
both pulsed directions; both `TMUX09`-tail attempts were liveness-aborted
before BRAM interpretation and promoted no path/codeword. The vendor
same-Port-A write positive remains vendor-only. Open hard-BRAM writes remain
unqualified.
