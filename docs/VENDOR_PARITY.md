...
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` adds exactly one Port-A read clock in the exercised read mode. One exact X13Y4 x2 OLD-mode write composition is also silicon-qualified | The 39 B4 rows across X13Y1..Y4 are experimental config encodings. `PACKEDMODE` has first-order effects in write-path and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null across read, write-path and dual-port. The earlier no-write images carried Yosys `emulate_read_first` DFFs on hard-BRAM write inputs. The production Qin pass now bypasses only structurally named, same-clock instances of that redundant emulation, and the inferred-BRAM library admits OLD mode only. A source-built pair with identical full all-one physical INIT, route, control, schedule, clocks and outputs differed only in its two DataInA LUTs: write-`00` read `0`, write-`11` read `3`, 1,500/1,500 samples per variant, FCB accepted, zero mapping debt. Uniform all-zero/all-one narrow INIT is deterministic; patterned narrow INIT mapping remains unqualified. General writes, byte enables, dual-port write semantics, other output-register modes, widths/sites, independent clocks, collisions/read-during-write, and NEW/NO_CHANGE remain unqualified. The CONFIG surface spans X13Y1..Y4; PLACEMENT remains X13Y4 only |
