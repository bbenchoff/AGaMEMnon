...
vendor-routed checkpoint. Since 2026-08-14 the default build also copies no
byte from the vendor-origin `fabric_default.bin` canvas — the base image is
synthesized from scratch out of the project's reverse-engineered databases
(which remain disclosed vendor-derived DATA), and the canvas ships only as a
non-loadable decode reference. Therefore “no vendor executable at runtime” and
“no canvas byte copied at build time” are accurate; “contains no vendor-derived
information” is not.
...
| V5 R2 routing campaign | 71,697/71,697 frozen live target rows have exact vendor-authored occupancy witnesses; 42,297 phantom rows are terminal, and 14 rows were catalogued as silicon-dead -- a classification since shown unreliable (a congestion-context artifact: six are board-proven to conduct and are now admitted, the remaining eight are conservatively blocked as unverified). The public research manifest records this closure | Witnessing is not selector admission, edge-specific encoding, or silicon conduction. Only six RMUX30 rows are reviewed under `experimental-strict`; `research-unsafe` may use broader vendor-derived/predicted knowledge without promoting it |
| Bitstream generation | Codec, CRC, 164-byte preamble, and supported feature overlays are open and regression-tested. The from-scratch generator is now the **default base**: it reproduces the canvas body **100% byte-exact (99,768/99,768)**, a design built on either base is **bit-identical** across the retained pack corpus, and the generated image **configures on silicon** (`FCB_STAT = 0x000f0002`; stale-CRC canvas rejected `0x00000040`) | The synthesized base is built from disclosed vendor-derived DATA tables, and the *function* of the unnamed reserved bit-lines and 15 `XXXX` spares is unproven (position and reset value only). `fabric_default.bin` stays shipped as a non-loadable decode reference |
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` adds exactly one Port-A read clock; bounded x2 `PORTB_OUTREG` adds exactly one Port-B read clock. One exact X13Y4 x2 OLD-mode write composition is also silicon-qualified | The 39 B4 rows across X13Y1..Y4 are experimental config encodings. `PACKEDMODE` has first-order effects in write-path and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null. The `PORTB_OUTREG` one-bit variant changed the retained dual-port set from `{0,2,4,6,8,A,C}` to `{2,4,8,E}` in 1,500 samples, exactly matching one extra Port-B clock. The earlier no-write images carried Yosys `emulate_read_first` DFFs on hard-BRAM write inputs. Qin now bypasses only structurally named, same-clock instances for uniform physical initializers; mixed/patterned designs retain their topology and NEW/NO_CHANGE remain behaviour-unqualified packing choices. The source-built write pair differed only in two DataInA LUTs: write-`00` read `0`, write-`11` read `3`, 1,500/1,500 samples per variant, FCB accepted, zero mapping debt. General writes, byte enables, broader dual-port semantics, other output-register modes, widths/sites, independent clocks, collisions, patterned INIT, and NEW/NO_CHANGE remain unqualified. The CONFIG surface spans X13Y1..Y4; PLACEMENT remains X13Y4 only |
...
| MCU External AHB | Public main has full HRDATA route recovery, a constant slave, a complete-byte bank with one controlled write wait, exact zero-extended 32-bit reads, aligned byte/halfword semantics, fail-closed non-SINGLE burst rejection, bounded address/control paths, and one-hot interrupt commands. A separate exact 16-bit held-scratch checkpoint passes aligned word/halfword and independent-byte state semantics, SRAM-churn retention, repeated reads, GPIO reset, write-side isolation of +0 against byte/halfword/word foreign offsets, low-16 aligned word-read isolation `[state,0,0,0]` at +0/+4/+8/+c, and CPU-visible aligned unsigned byte/halfword lane selection and zero extension. A hash-bound derivative moves that exact scratch to public offset +4 and passes three complete hardware runs, but does not yet coexist with ID/counter/W1C | Hard reset, alternate bus clocks, generic direct-D, full 16-bit public-bank integration, raw upper-HRDATA characterization, higher/full-window decode, misaligned and signed loads, arbitrary placement/width, complete protocol behavior, AHB master/DMA, and broader peripherals remain; HRESP-to-fault is retired and misaligned CPU accesses are characterized hard-core faults |
