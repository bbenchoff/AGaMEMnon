...
| BRAM | Behavior-qualified X13Y4 open-flow read subset: x18 Port A, x2 Port B, documented x9 projections/bundle, and one opt-in fresh-source x18 full-depth composition passing 512/512 words with all nine address lanes. Four hash-bound profiles qualify one fixed-address registered-source write A/B through `TMUX09 -> KMUX03`; exact replay and fail-closed source-to-route builds are supported. Structural metadata and vendor oracles cover X13Y1..Y4 | Fresh Y1 remains partial (`0x100` address mask), Y2/Y3 are unqualified, and production placement remains X13Y4. Ordinary/inferred writes, general dual-port operation, widths, independent clocks, collisions, and general modes remain unqualified |
