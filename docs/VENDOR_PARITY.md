...
| BRAM | Behavior-qualified X13Y4 read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` additionally has measured behaviour — exactly one BRAM clock of added Port-A read latency at X13Y4, in the one exercised read mode | The 39 B4 rows across X13Y1..Y4 are experimental config encodings; only `PORTA_OUTREG` has any behavioural measurement, and `PACKEDMODE`/`CLKMODE` have only a bounded null in that same read mode. General writes, byte enables as behavior, dual-port operation, output-register behaviour on Port B or in other modes, width/mode combinations, independent clocks, collisions/read-during-write, high-address breadth, and other-site behavior remain unqualified. Note also that the X13Y1..Y4 scope is the CONFIG surface (`agamemnon/engine/pips_bram_pll.csv`); the PLACEMENT surface is X13Y4 ONLY |
