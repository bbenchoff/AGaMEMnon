...
| BRAM | Behavior-qualified X13Y4 open-flow read subset: x18 Port A, x2 Port B, and documented x9 projections/bundle. `PORTA_OUTREG` adds exactly one Port-A read clock; bounded x2 `PORTB_OUTREG` adds exactly one Port-B read clock. Four hash-bound x18 profiles qualify one fixed-address registered-source write A/B through `TMUX09 -> KMUX03`; exact replay and fail-closed source-to-route builds are supported. Exact structural BEL/frame metadata covers X13Y1..Y4. Vendor-routed zero-LUT oracles read distinct Port-A marker bytes and then pass all 512 x18 Port-A addresses at all four arrays simultaneously with zero first-read, settled-read, and upper-half errors on silicon | The 39 B4 rows across X13Y1..Y4 are experimental config encodings. The broad four-site route harvest is reference-only; production routing/placement remains X13Y4 until open-flow corridors are independently qualified. `PACKEDMODE` has first-order effects in bounded write-path-shaped and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null. Ordinary/inferred writes, WeA mechanism/general routing, broader dual-port operation, other output-register modes, widths, independent clocks, collisions, patterned INIT, and NEW/NO_CHANGE remain unqualified |
...
both pulsed directions. Two early `TMUX09` attempts were liveness-aborted; a
later registered-source four-arm matrix corrected the apparatus. Its four
