...
| Fabric local interrupts | Silicon-qualified routing/cause and cause-17/18/19 command subset | Four distinct sources route simultaneously to `local_int[3:0]`; lanes independently deliver local causes 16–19 with the matching `mip` bit. Separate AHB write-command images qualify retained pending, mask/unmask, acknowledge, two re-arms, masked hold, and GPIO reset for causes 17–19. Reads return zero; state readback and cause-16 command-state widening remain open |
...
2. Four distinct fabric sources route simultaneously to `local_int[3:0]` and
   ...
   For causes 17 through 19, AHB write-command images now retain a masked pending event,
   ...
   reset. Their traps report `mcause=0x80000011`, `0x80000012`, and
   `0x80000013` respectively. Reads intentionally return zero, pre-`mie` local
   `mip` visibility is not claimed, and cause-16 command-state widening
   remains open.
