...
| BRAM | Silicon-qualified subset | X13Y4 read support includes the established x18/x9 Port-A and bounded x2 Port-B subsets plus a new opt-in fresh-source x18 full-depth oracle: all 512 words pass with all nine address lanes observed, zero first/settled/upper-half errors, and zero selector debt under `AGAMEMNON_BRAM_SITE_READ_PATHS=1`. Four hash-bound X13Y4 registered-source profiles still qualify only one fixed-address write A/B through `TMUX09 -> KMUX03`; generic/inferred writes remain unqualified. Vendor oracles prove simultaneous hard-array behavior at X13Y1..Y4, but fresh Y1 is only partially live (address mask `0x100`) and Y2/Y3 are unqualified, so production placement remains X13Y4 and the site-read profile remains experimental. Widths, ports, independent clocks, modes, and collisions are not generalized. |
...
That closes one bounded source-to-route result at X13Y4. The fresh parity-ROM
image passes all 512 x18 Port-A words with zero first-read, settled-read, or
upper-half errors and observes every power-of-two address lane (`0x1ff`). The
one-bit HSE negative and two missing-address-buffer negative establish the
causal boundary. X13Y1 is only partially live after relocating ROM control: it
observes address mask `0x100` and fails 256/512 parity comparisons. A broad
vendor hard-field replay made it worse and was rejected. X13Y2/Y3 remain
unqualified, so arbitrary-site placement and release admission remain open.
