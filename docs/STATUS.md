...
mode (x18 Port-A read, identity ROM contents, 4-bit fabric address, Port-B
  unused, single clock domain). `PACKEDMODE` has measured first-order behaviour in bounded write-path-shaped and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null. `PORTB_OUTREG` adds exactly one Port-B read clock in the retained X13Y4 x2 single-clock oracle. Direct hard-output probes refute the former source-built write claim: INIT=1/write-`00` stayed `11`, and INIT=0/write-`11` stayed `00`; the earlier F0/F3 result was fabric-side wrapper output. A later conflict-free x18 A/B independently proved both Port-A and Port-B clock/address/read liveness, held Port-A `ClkEn0` and both byte enables high, retained the vendor `CLKMODE=10` and X15-selected WeA terminal, and changed only WeA low versus high; both INIT polarities still retained INIT. Earlier hand-patched full-control images are void because distinct nets simultaneously owned RMUX60/RMUX84. Bitgen now rejects that composition. Production no longer bypasses `emulate_read_first` automatically. No hard-BRAM write is qualified, and no WeA polarity or mechanism is inferred; exact terminal-control semantics, patterned INIT, other widths/sites/modes/clocks, and collisions remain open.
