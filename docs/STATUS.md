...
| External AHB register classes | Silicon-qualified subset | The complete public profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled write wait, exact zero-extended 32-bit reads, aligned byte/halfword semantics, and fail-closed non-SINGLE bursts. Separately, one exact 16-bit held-scratch checkpoint is silicon-qualified for aligned word write/hold/read, retention across SRAM churn and repeated reads, one wait, GPIO reset, and write-commit isolation of +0 against aligned writes to +4/+8/+c through HADDR[3:2]. Its foreign reads deliberately alias +0, so read/full address decoding remains open. It uses external LUT-buffer feedback with no direct-D cells and repacks byte-identically. Subword access, upper HRDATA lanes, bursts, arbitrary placement/width, and integration into the public bank remain open; deterministic MCU exception behavior from HRESP is retired on L48 |
