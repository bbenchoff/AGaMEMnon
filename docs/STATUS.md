...
Baseline provenance: emitted images are assembled onto a
...
| Physical outputs | Silicon-qualified L48 subset | Characterized header outputs, left-edge PIN_25 through PIN_28, and **all ten TOP-edge decimal physical leads PIN_10 through PIN_19**, built by the ordinary CLI with `--pcf`. The closing PIN_10/PIN_11 singles and same-tile pair toggle only their intended GP4/GP1 leads under both Pico pulls, with zero selector debt; the production pair is byte-identical to the measured candidate and its retained route repacks byte-identically. These names are decimal L48 package-lead labels, not hexadecimal indices. The left-edge four also reproduce from the ordinary CLI (`agamemnon build qualification/left_edge_outputs.v --pcf qualification/left_edge_outputs_L48.pcf --research-unsafe`, image sha256 `a63ab5bc26bb4852555fb93863f065ba020564ec77e801cd4d67d4bcf865aba3`, 35 pips / 0 unmapped / 0 predicted / 0 legacy-abs, Pico GP12 404,383 Hz, GP13 405,612 Hz, GP16 405,168 Hz, GP17 411,144 Hz with GP8 (PIN_18, undriven) 0 Hz as the negative control, FCB 0x000f0002; bench observation, ledger row pending). Flow caveat: that ordinary-CLI recipe is the Python-architecture (non-`--uarch`) PCF placer, which composes experimental options and needs `--research-unsafe`. `agamemnon build <source> --uarch --pcf <pcf>` (no `--research-unsafe`) also builds release-strict, zero-unmapped/predicted/legacy-selector images for the left-edge four and nine of the ten top-edge leads (all but PIN_15) -- `AGAMEMNON_VENDOR_OUT_SLICE` is now release-admitted for exactly the four qualified presentations, value-gated so nothing else is -- and every one of those images FCB-configured the real device to 0x000f0002 over a non-destructive SRAM session (`io_evidence.jsonl` trial `pad-uarch-pcf-release-strict-vehicle-config-accept-20260817`). The Pico toggle re-witness of that vehicle's own images is closed for all ten (`io_evidence.jsonl` trial `pad-uarch-pcf-toggle-rewitness-20260817`): each of the left-edge four and the nine top-edge outputs toggles under both Pico pulls on exactly its intended lead and no other. The toggle/electrical re-witness for this vehicle's own five qualified-input demonstration builds is closed, matching the ten output images: under the corrected active-drive procedure all five track their driven input correctly on both drive levels, reproducibly across two independent full sweeps (`io_evidence.jsonl` trial `pad-uarch-input-rewitness-closed-silicon-20260817`). An initial negative (fixed-level reads regardless of drive) did not reproduce under a fresh session and is attributed to a session/rig-specific condition, not a vehicle, procedure, or bitstream defect (byte-level and route-level identity with the qualified images is established; an SRAM stack/image-address-overlap code smell in `agamemnon/program.py`'s SRAM_SP was tested directly at its as-shipped value and did not reproduce the negative either). PIN_15 as an output still needs `--research-unsafe` under either vehicle |
...
| External AHB write | Silicon-qualified subset | All 32 MCU write-data lanes in protocol-valid four-bit groups, plus one exact simultaneous 16-lane posted-capture checkpoint with source-to-route replay; not a register bank and not one simultaneous 32-bit capture |
...
| External AHB bus clock | Silicon-qualified subset | Pure-open default `bus_clk = sys_gck` delivery qualifies direct-D sites X14Y11 slice4 through slice7, an eight-state three-bit counter, and a 16-bit LFSR with 500 distinct reads. Across three runs and 45 intervals the LFSR advances exactly one step per undivided MTIME tick — a 1:1 ratio, which is the qualified quantity; the absolute rate is an [open question](MCU_CLOCKS.md#external-ahb-bus-clock): MTIME is measured at 14.08 MHz, and a 10 MHz rate assumes a 10 MHz HSI/MTIME. A GPIO4.1-fed synchronous reset held all 16 state bits at zero and re-armed in three runs. Hard `MCU_RESETN`, PLL3 BUSCLK, unrestricted direct-D lowering, and the fourth binary carry cone remain unqualified |
| External AHB constant slave | **Silicon-qualified for the pinned checkpoint (re-verified 2026-08-19); fresh builds not guaranteed** | Constant-ready, OKAY-only combinational endpoint; the 2026-08-02 qualification read `0x4147414d`, writes complete without effect; no wait/error/register-bank claim. T22–T24 found a fresh build of the *identical source* at then-current HEAD (`bitstream_sha256 fc6919c2…`) instead read `0x795fe3dd` (10 of 32 bits wrong) — reproduced 96/96 on an L64 unit **and** the L48 reference board, not unit/package-specific. T26 traced all 10 wrong bits to a shared `X14Y8` RMUX→IMUX→RMUX detour — the same detour class 48cda69 already flagged as a separate open defect, and **still open**. (T26 also proposed an exact chipdb tuple for `X14Y11_RMUX03->X13Y11_BBMUXE09`; A1b **retracted** it on board evidence — it reads a live bit 22 as 0 in the public32 designs, so 48cda69's fallback was correct.) nextpnr's route for this design differs from the pinned 2026-08-02 build purely from chipdb growth, so a **fresh `agamemnon build --uarch` of this exact design is not currently guaranteed correct**. `qualification/mcu_ahb_constant_slave_routed.json` is re-pinned to the retained 2026-08-02 (f705d28) routed netlist; packing it with current bitgen yields `bitstream_sha256 3ef719a0…`, which was read on the physical L48 reference board (72/72 direct + 6/6 mailbox reads of `0x4147414d`, FCB_STAT `0x000f0002` and DEVICE_ID `0x40200001` every cycle, SRAM-only). Cite `0x4147414d` only for that pinned artifact, not for a from-scratch rebuild. Detail: [CONDUCTION_REFRAME_STATUS.md](CONDUCTION_REFRAME_STATUS.md), `qualification/mcu_ahb_constant_slave_evidence.jsonl` (trials `2026-08-18-t26-regression-fix` and `2026-08-19-a1b-codeword-matched-ab`) |
| External AHB register classes | Silicon-qualified exact L48 subset | The default SDK profile composes canonical ID32 `0x4147414d` at +0, zero-extended scratch16 at +4, counter3 at +8, and W1C1 at +c. Three sequential full-map SRAM-only runs pass exact LW values, every ID byte/halfword lane, unsigned-load zero extension, scratch word/halfword and independent byte +4/+5 semantics, foreign-address isolation, retention, coexistence, GPIO4.1 reset, all eight counter states, and the qualification-only W1C set/clear/set-priority matrix. A separately selectable exact GPIO5-W1C derivative removes the bit1 self-test set hook and uses MCU GPIO5 DATA0/OUT_EN0 as an independently routed sustained-level source. One base negative (`status_errors=162`), one OR control (`2`), and three production runs (`0`) preserve every non-status check and prove low/hold/clear, high/set-priority, and reset dominance. GPIO5 is software-controlled qualification stimulus, not a package-pin input or autonomous asynchronous event. The deterministic composers, structural replays, raw/compressed images, and causal controls are hash-pinned and strict-clean. A second exact profile qualifies one reset-rearmed HCLK-synchronous counter event through distinct negative/OR/production signatures while preserving the public32 matrix. The public16 and older complete-byte profiles remain separately available. A generic application-owned status socket, misaligned and signed loads, higher/full-window decode, bursts, arbitrary placement/width, other packages, and a generic register-bank generator remain open; deterministic MCU exception behavior from HRESP is retired on L48. **Caveat (2026-08-18, updated by T26):** this profile replays a hash-bound, pinned exact L48 map (`agamemnon build` rejects source/routed-netlist/board/device/output-hash drift for it per docs/USAGE.md), which is architecturally the same style of fix T26 applied to the constant slave above, so it is immune to the fresh-nextpnr-route drift T26 found (its routed JSON is a checked-in pin, confirmed 2026-08-19). It was also the design that **arbitrated** T26's proposed chipdb row: this profile's own bit 22 is live, so a matched board A/B here (`bc338504…` PASS with `(2,4)` versus `8673ae0d…` FAIL with `(1,6)`, ID32 `0x4147414d` → `0x4107414d`) refuted that row and it was dropped. This row's evidence is unaffected and was re-confirmed on the board on 2026-08-19. See CONDUCTION_REFRAME_STATUS.md, A1b addendum. |
...
5. The register-window soft-UART core and fail-closed protocol boundary pass
   ...
   TX/RX binding remain unqualified. Hard-UART0 TX/RX fabric routes are now
   separately silicon-qualified (PIN_10 TX, PIN_31 RX, and the PIN_30/PIN_31
   DAP-CDC duplex and line-mode compositions in the hard-peripherals table).
...
`RMUX43 -> BBMUXE06` selector `{1,6}`; the source-only fallback
`{0,6}` is the defect. An atomic qualified-corridor reservation then routes
...
The default `--uarch` build uses a three-tier admission model. Tier 1
(witnessed) edges — conduction evidence at the exact position — are admitted
silently. Tier 2 (encoding-certain) edges — no conduction witness at this
position, but the selector codeword is certain by clean-physical,
unanimous-relative, or a zero-counterexample closed form — are admitted and
recorded in the build's `<output>.confidence.json` manifest. Tier 3
(encoding-ambiguous) edges — conflicting or unresolved selector keys — are
refused. `agamemnon build --release-strict` restricts the build to tier 1
alone. The model is specified in [routing admission](ROUTING_ADMISSION.md).

The device database retains a fail-closed negative-evidence mechanism for routing
edges; its set is empty. The fourteen former rows were classified
from negative silicon trials that were not truly isolated, so that classification is
unreliable: they came from one large,
congested MCU-exit design, and the failures were a congestion-context effect
mis-attributed to individual edges. On silicon, **all fourteen** catalogued edges -- `RMUX21@(14,10)->RMUX87@(14,8)`,
...
at `0xfffffffb`. The static negatives remain valid, while their constant
values describe readback visibility/default state rather than initialized
...
qualifying HADDR11/AddressA12 through X14Y7 slice3. The static observations
remain valid, but their interpretation as dead INIT/address behavior does not hold; the isolated q3 constant was specifically caused by incoherent
...
and INIT=0/write-`11` stayed `00`; the F0/F3 result is fabric-side
...
widths, ports, sites and schedules remain unqualified. Hand-patched full-control images are void because distinct
...
| UART0 external TX/RX | Silicon-qualified subset | Separate images qualify physical L48 PIN_10 TX and PIN_31 RX at requested 9600, 38400, and 115200 baud. A combined zero-LUT image routes GPIO7.6/UART0_UARTTXD → PIN_30 and PIN_31 → GPIO6.1/UART0_UARTRXD; three fresh full-load runs transferred 4096 exact bytes each way simultaneously at all three rates with zero target error/mismatch and elapsed time near one ideal wire direction. At 38400, 7E1/8E1/8O1/8N2 each transfer 256 bytes both ways; matched 8E1 produces 0/64 parity flags while target-8E1/host-8O1 produces 64/64 with intact payload and no framing/break/overrun flags. The original wrong-clock TX run (~560 baud when firmware passed 248 MHz for requested 9600) remains a useful negative. This qualifies those exact polled/FIFO-enabled compositions in the inherited HSI state, not sub-percent absolute accuracy, flow control, FIFO saturation, framing/break/overrun injection, 5/6-bit or mark/space modes, UART1–4, another oscillator/board, dynamic clock switching, or the mask-ROM protocol |
| I2C0 master write/read | Silicon-qualified subset | Active open-drain interoperability on physical L48 pads (SDA PIN_11, SCL PIN_15): an RP2350 slave at `0x55` supplied pull-ups and first qualified separate one-byte write/read. Three fresh register-style runs then wrote `2A A6` without STOP, changed direction with repeated START, and read exact `5A C3 7E` while the master drove ACK/ACK/NACK+STOP; every HAL status was zero. Silicon reports the intentional terminal master NACK as `SR.RXNACK` (`SR=0x81`), so the repaired last-byte SDK path treats that one expected status as success. The 288-transaction no-slave framing/NACK capture remains valid. Arbitrary lengths, STOP-delimited multi-byte writes, clock stretching, 10-bit addressing, arbitration/multimaster, I2C1, interrupts/DMA, and absolute SCL timing remain unqualified |
...
idles recessive-high and this bench has no transceiver. Register-level configuration
and transmit-completion are recorded as a bounded `partial` row in
`qualification/hard_peripheral_evidence.jsonl` (2026-08-14, workbench-firmware
stimulus); that row itself records that no CAN bits were observed on a wire,
so no on-wire CAN claim is made. USB host/OTG and the Ethernet MAC
...
[ANALOG_FABRIC_BOUNDARY.md](ANALOG_FABRIC_BOUNDARY.md) — their DAC-to-ADC
loopback and CMP0-unit-1 observations are recorded in
`qualification/hard_peripheral_evidence.jsonl` (2026-08-14), though those rows
name workbench-only stimulus firmware and are not reproducible from checked-in
source. No speculative driver is shipped
