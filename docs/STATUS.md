...
| External AHB register classes | Silicon-qualified subset | The complete public profile remains the 8-bit ID/scratch/counter/W1C bank with GPIO4.1 reset, one controlled write wait, exact zero-extended 32-bit reads, aligned byte/halfword semantics, and fail-closed non-SINGLE bursts. Separately, one exact 16-bit held-scratch checkpoint is silicon-qualified for aligned word and halfword +0 writes, independent byte writes at +0/+1, rejection of byte +2/+3, aligned halfwords +2/+4/+8/+c and aligned words +4/+8/+c, retention, overwrite, one wait, GPIO reset, low-16 aligned word reads at +0/+4/+8/+c as `[state,0,0,0]`, and CPU-visible aligned unsigned byte/halfword lane selection and zero extension. Four 100-pattern write runs, a ten-run causal read-decode matrix, and three 32-pattern/128-observation subword-read runs passed. A hash-bound two-INIT derivative moves that exact scratch to public offset +4; three further 32-pattern/160-observation runs pass word/halfword +4, bytes +4/+5, foreign-address rejection, decoded reads, retention and reset. It uses external LUT-buffer feedback with no direct-D cells and repacks byte-identically. The +4 result is an isolated public-scratch-shaped object, not coexistence with ID/counter/W1C. Misaligned and signed loads, raw upper HRDATA behavior, higher/full-window decode, bursts, arbitrary placement/width, and full public-bank integration remain open; deterministic MCU exception behavior from HRESP is retired on L48 |
