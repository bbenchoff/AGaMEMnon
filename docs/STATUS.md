...
| PLL | Byte-exact emitted subset; silicon-qualified subset | Emission is fail-closed to `(SYSCLK,HSE)` pairs `(100,8)`, `(50,8)`, `(25,8)`, `(10,8)`, `(100,16)`, `(60,8)`, and `(100,12)` MHz. `(60,8)`, `(50,8)`, `(25,8)`, and `(10,8)` are silicon-frequency-qualified: an SRAM run switches the MCU to that PLL and measures the effective clock (60/50/25/10 MHz) against the host wall-clock. `(100,8)` and `(100,16)` remain preamble/timing-qualified only (their 100 MHz SRAM trial is deferred — the DAP link would run at 100 MHz during readback); `(100,12)` cannot be exercised on the 8 MHz-HSE reference board. |
...
60 MHz (`qualification/pll_freq_evidence.jsonl`). `(50,8)`, `(25,8)`, and
`(10,8)` qualify identically: two-window SRAM runs extrapolate to exactly 50.0,
25.0, and 10.0 MHz (the same ~26–27 ms fixed host offset), with the PLL locked
and selected and the DAP link stable at ≤60 MHz. `(10,8)` is numerically equal
to the 10 MHz HSI reference but is confirmed PLL-driven by the CLK-register
lock/select bits. This qualifies the `(60,8)`/`(50,8)`/`(25,8)`/`(10,8)` output
frequencies only, not other outputs, phase, duty, feedback, or bypass.
`(100,8)` and `(100,16)` remain preamble/timing-qualified pending a 100 MHz
trial, deferred because the DAP runs at the PLL clock during halt/readback and a
lost link at 100 MHz needs a physical power cycle. `(100,12)` still cannot be
exercised on the 8 MHz-HSE reference board, so it carries only an
encoding/timing claim. The qualified
