...
| External AHB register classes | Silicon-qualified exact L48 subset | The default SDK profile now composes canonical ID32 `0x4147414d` at +0, zero-extended scratch16 at +4, counter3 at +8, and W1C1 at +c. Three sequential full-map SRAM-only runs pass exact LW values, every ID byte/halfword lane, unsigned-load zero extension, scratch word/halfword and independent byte +4/+5 semantics, foreign-address isolation, retention, coexistence, GPIO4.1 reset, all eight counter states, and the qualification-only W1C set/clear/set-priority matrix. The deterministic composer widens the preserved public16 checkpoint with 16 exact HRDATA exits and one ID selector; the structural replay plus raw/compressed images are hash-pinned and strict-clean. The public16 and older complete-byte profiles remain separately available. Production status-set ingress, misaligned and signed loads, higher/full-window decode, bursts, arbitrary placement/width, other packages, and a generic register-bank generator remain open; deterministic MCU exception behavior from HRESP is retired on L48 |
