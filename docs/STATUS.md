...
| UART0 external TX/RX | Silicon-qualified subset | Separate images qualify physical L48 PIN_10 TX and PIN_31 RX at requested 9600, 38400, and 115200 baud. A combined zero-LUT image routes GPIO7.6/UART0_UARTTXD → PIN_30 and PIN_31 → GPIO6.1/UART0_UARTRXD; with the DAP CDC endpoint, three fresh full-load runs transferred 4096 exact bytes each way simultaneously at all three rates with zero target error/mismatch and elapsed time near one ideal wire direction. The original wrong-clock TX run (~560 baud when firmware passed 248 MHz for requested 9600) remains a useful negative. This qualifies exact 8-N-1 polled/FIFO-enabled full duplex in the inherited HSI state, not sub-percent absolute accuracy, flow control, FIFO saturation/error injection, UART1–4, another oscillator/board, dynamic clock switching, or the mask-ROM protocol |
