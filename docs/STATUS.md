...
| External AHB register classes | Silicon-qualified subset | One pure-open image integrates immutable ID low byte `0x4d` at offset 0, reset-zero writable scratch byte at offset 4 over all 256 values, a read-only lower-three-bit counter at offset 8, and one-bit W1C status at offset C. The counter has a constant nonzero fixed-read cadence plus an independent phase-swept window covering all eight states and ignores writes. Status passes software-set, repeated hold, zero-write no-op, clear, wrong-offset isolation, re-arm, simultaneous set/clear with set priority, and scratch preservation. Integrated reset, waits/errors, and byte/halfword semantics remain open |
...
1. Default External-AHB `bus_clk = sys_gck` delivery is timer-qualified at
   ...
   cross-register preservation. Integrated reset, waits, errors, and byte/
   halfword semantics remain open. HWDATA0 is
