...
| Fabric local interrupts | Silicon-qualified routing/cause and four-lane command subsets | Four distinct sources route simultaneously to `local_int[3:0]`; lanes independently deliver local causes 16–19 with the matching `mip` bit. Separate AHB write-command images qualify retained pending, mask/unmask, acknowledge, two re-arms, masked hold, and GPIO reset for every cause. Cause 16 uses an explicit composite-command class with offsets 4/C as aliases. Reads return zero; state readback and one-image all-four integration remain open |
...
2. Four distinct fabric sources route simultaneously to `local_int[3:0]` and
   ...
   For causes 16 through 19, AHB write-command images now retain a masked pending event,
   delivers it after mask enable, acknowledges it, re-arms twice, holds a
   third event while masked, deliver it after unmask, and clear on GPIO4.1
   reset. Their traps report `mcause=0x80000010` through `0x80000013`.
   Cause 16 uses commands 00=mask-off/hold, 01=mask-on/ack,
   10=mask-off/set, and 11=mask-on/set at explicit offset-4/C aliases. Reads
   intentionally return zero, pre-`mie` local `mip` visibility is not claimed,
   and one-image all-four integration remains open.
