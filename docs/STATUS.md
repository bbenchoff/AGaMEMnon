...
| BRAM | Silicon-qualified subset | One x18 Port-A path, one x2 Port-B read/control path, and X13Y4 read-only x9 with visible data bits0..7 plus an exact HADDR11/AddressA12 word-0/512 projection; the backend represents independent A/B ports |
...
data surface is therefore bits `[7:0]` within these exact projections.
Full-width INIT permutations also make logical data bits6 and 7 / physical
DataOutA15 and DataOutA16 match aligned word-address bits0 and 1 respectively
for 256/256 reads through their exact direct corridors. Data bit8, the remaining high-address lanes/range, writes,
...
terminal-identity defect. Other BRAM tiles, data bit8, the remaining
