...
| BRAM | Silicon-qualified subset | One x18 Port-A path, one x2 Port-B read/control path, all nine X13Y4 read-only x9 data bits through exact per-lane projections, a simultaneous strict-open x9 bundle, and the exact 1024-aligned-word HADDR[11:2] address bundle. Four hash-bound X13Y4 x18 registered-source profiles additionally qualify one fixed-address write A/B through `TMUX09 -> KMUX03`; both exact replay and explicit `build --uarch --qualified-bram-write` source-to-route are supported. Fresh builds collision-audit/canonicalize the measured trees and must reproduce the exact silicon image hashes. Edited/inferred/generic writes remain unqualified. Separately, 39 configuration rows across X13Y1..Y4 are admitted only under `experimental-strict`; the admissions themselves are config-encoding, not behavioral claims. `PORTA_OUTREG` adds exactly one Port-A read clock in the exercised read mode; `PORTB_OUTREG` adds exactly one Port-B read clock in the bounded X13Y4 x2 single-clock dual-port composition. `PACKEDMODE` has a measured first-order effect in bounded write-path and dual-port oracles, with no mechanism claimed; `CLKMODE` remains a bounded null across the three tested compositions. Note the two surfaces: the CONFIG surface covers X13Y1 through X13Y4, while PLACEMENT is X13Y4 ONLY |
...
| SPI0 master transmit + RX lane | Silicon-qualified subset | MOSI/SCK/CSN on physical L48 pads: 233/233 words all `0x55` plus `11 22 33 44` with 108 matches; MSB-first and CS-framed. The TX API left-justifies sub-word payloads. On a second exact image with IO1 on PIN_17, sampled-high receive widths 1–4 returned raw `A50000FF`, `A500FFFF`, `A5FFFFFF`, `FFFFFFFF`; the repaired read API masks stale upper state and returns the valid low-order bytes. Divider 2–256 readback and monotonic relative timing are also qualified. Absolute SCK/reference frequency, arbitrary slave-driven RX values and multi-byte order, full duplex interoperability, DUAL/QUAD, DMA, SPI1, and non-power-of-two dividers remain unqualified |
