...
| External AHB register classes | Silicon-qualified exact L48 subset | The default SDK profile now composes ID8 `0x4d` at +0, held scratch16 at +4, counter3 at +8, and W1C1 at +c. Four sequential SRAM-only runs pass aligned scratch word/halfword and independent byte +4/+5 semantics, representative foreign-address isolation, decoded reads, retention, coexistence, GPIO4.1 reset, all eight counter states, and a qualification-only W1C bit1 set/bit0 clear hook with set priority. The deterministic composer preserves the qualified +4 storage spine, and the structural replay plus raw/compressed images are hash-pinned. The older complete-byte profile remains separately available with its exact-zero read and fail-closed non-SINGLE evidence. Raw HRDATA[31:16], canonical 32-bit ID, production status-set ingress, misaligned and signed loads, higher/full-window decode, bursts on the new composition, arbitrary placement/width, other packages, and a generic 32-bit ABI remain open; deterministic MCU exception behavior from HRESP is retired on L48 |
