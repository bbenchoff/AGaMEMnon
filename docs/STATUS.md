...
wrapper output. Four later hash-bound X13Y4 x18 registered-source checkpoints
causally show the low arm retaining INIT and the high arm reaching opposite
`DataIn` through `TMUX09 -> KMUX03`. Those exact images are exposed only by
`agamemnon pack <packaged checkpoint> <out> --qualified-checkpoint PROFILE`;
ordinary source-to-route/inferred writes, WeA mechanism, general TMUX/KMUX
routing, other addresses, widths, ports, sites and schedules remain
unqualified. Earlier hand-patched full-control images are void because distinct
