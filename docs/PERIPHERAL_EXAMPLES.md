...
| UART | 5 hard instances plus open polling HAL and internal-loopback example | `uart_tx.v`, 8N1 transmitter | Retained UART0 loopback and PIN_30/PIN_31 duplex profiles remain exact evidence. The campaign additionally qualifies fixed UART0/1/2 TX contracts on PIN_10 at nominal 9600/38400/115200. UART3/4 TX, campaign RX for UART0–4, arbitrary payload/framing, flow control, break, interrupt/DMA, other routes/packages, and absolute calibration remain open; FPGA combined simulation passes |
| SPI | 2 hard instances plus open eight-phase polling HAL | `spi_master.v`, one-byte mode-0 master | SPI0 and SPI1 TX pass bounded mode-3, 1–4-byte, documented-divider and raw byte-order contracts on exact L48 routes. A retained older SPI0 receive image remains exact evidence, but both new typed SPI0/SPI1 MISO compositions returned `0xffffffff`; typed MISO now fails closed under `VP-AGM-008`. No generic RX/duplex, dual/quad, DMA/POLL/interrupt, other modes/routes, or absolute SCK claim. FPGA loopback simulation returns `0xA5` |
| I2C | 2 hard instances plus open polling master HAL | `i2c_writer.v`, one-byte single-master write | I2C0 and I2C1 pass the exact address-`0x55` write `2A A6`, repeated-START read `5A C3 7E`, ACK/ACK/NACK+STOP contract on exact L48 open-drain routes. I2C0 also passes one four-point 500 us stretch profile. Other lengths, longer/unbounded stretching, 10-bit addressing, arbitration, simultaneous controllers, interrupt/DMA, and electrical/timing margins remain open. FPGA ACK-path simulation passes |
| CAN | 1 hard CAN 2.0 instance | no protocol-complete soft CAN block yet | **Not wire-qualified.** Register-level `partial` row ledgered 2026-08-14; no CAN bits observed on a wire; needs an external CAN transceiver |
...
| ADC/DAC/comparator | hard analog blocks | cannot be synthesized from digital LUT RTL | A one-shot/static subset has been observed on the bench over the External-AHB window, but only through the **vendor `analog_ip` macro** that the open flow does not emit (ledgered 2026-08-14, workbench-only stimulus); see [ANALOG_FABRIC_BOUNDARY.md](ANALOG_FABRIC_BOUNDARY.md) |
