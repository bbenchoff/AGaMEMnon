...
| I2C | 2 hard instances plus open polling master HAL | `i2c_writer.v`, one-byte single-master write | I2C0 active open-drain single-byte write/read is silicon-qualified on exact L48 pads: an RP2350 slave at `0x55` ACKed address/write `0xA6` and returned `0x5A`. Multi-byte transfers, repeated START, stretching, I2C1, and absolute SCL timing remain open. FPGA ACK-path simulation passes |
