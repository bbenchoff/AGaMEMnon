...
| SPI | 2 hard instances plus open eight-phase polling HAL | `spi_master.v`, one-byte mode-0 master | SPI0 and SPI1 TX pass bounded mode-3, 1–4-byte, documented-divider and raw byte-order contracts on exact L48 routes. A retained older SPI0 receive image remains exact evidence, and corrected typed SPI0/SPI1 MISO now passes the [bounded receive contract](SPI_RECEIVE_QUALIFICATION.md). No generic RX/duplex, dual/quad, DMA/POLL/interrupt, other modes/routes, or absolute SCK claim. FPGA loopback simulation returns `0xA5` |
