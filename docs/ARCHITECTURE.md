...
## The engine on top — nextpnr-generic, MCU edge, flash-boot

The place-and-route engine is built and silicon-proven. `agamemnon/engine/arch.py` is a `nextpnr-generic` adapter that builds the real AGRV2K architecture (50,046 wires, 2,112 LE bels, the routing pips, plus MCU-edge / IO / clock bels) from the shipped chipdb, and `nextpnr-generic` packs, places, and routes combinational, sequential, and MCU-instance designs on it. Sequential function is proven on silicon (flip-flops toggle; register-select solved), clock nets route to arbitrary tiles (the open HSE→PLL→global-clock source is emitted directly by bitgen, no vendor clocked baseline), and the MCU↔fabric bridge (`alta_mcu`/`alta_rv3200`) carries both per-bit GPIO (4-bit loopback, 16/16 combos) and the External-AHB memory bus (`*0x60000000` write path). Finally, an open bitstream boots itself from flash: written to the fabric-config region, the boot ROM decompresses and configures it at power-on with no debugger in the loop. Synthesis is stock `yosys` with the AGRV2K LUT4 cell library (`agamemnon/synth/`). See `STATUS.md` for the done/remaining breakdown and `BITSTREAM_FORMAT.md` for the bit-level format.