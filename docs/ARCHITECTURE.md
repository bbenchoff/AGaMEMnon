...
- `agamemnon/chipdb/<file>` — a shipped device-database file (the recovered chip data; the two largest are git-lfs).
...
- **Packages / device model.** One AGRV2K die is offered in **4 QFN packages — L100 / L64 / L48 / Q32** — differing *only* in which perimeter IO pins are bonded out (the core LUT/FF/RMUX mesh is identical). AGaMEMnon transcribes the per-package legal-pin sets verbatim from the vendor `CHIP_INFO` (see `agamemnon/engine/device.py`) and uses them as a **front-end pin-NUMBER legality gate** — a design that *declares* a `PIN_n` the selected package does not bond is rejected (`device.check_pin`). The default device is **AGRV2KL48** (the dev board). Select with `AGAMEMNON_DEVICE`. User-IO counts: L100=79, L64=49, L48=34, Q32=26. Package selection **does not** currently prune the fabric IOB/pad bels the router may use: precise per-package *physical* pad restriction needs the `PIN_n → IOTILE` bond map, which lives in `af.exe` and is **a documented follow-up** (not yet extracted). Until then all fabric pads are exposed regardless of package, and the gate is pin-declaration legality only.
...
- **Sel/edge encoding** — a used edge lights a 2-hot sel pair; the destination CFG group is `dst_node_idx // {RMUX:6, IMUX:4, OMUX:3}`. The inter-tile RMUX mesh is closed-form; the intra-tile OMUX→IMUX crossbar is completed by validated cross-tile replication; the MCU-edge BBMUXS exit is a per-source instance-independent pair table. Byte-validated against `af.exe` at **~99% with a false-positive rate of zero** — the encoder emits only what it can prove, so the residual is dropped/approximated pips, never wrong bits. **File:** `chipdb/sel_map.json`.

## The engine on top — nextpnr + the `agrv2k` uarch, MCU edge, flash-boot

The place-and-route engine is built and silicon-proven. The backend is nextpnr driven by our own microarchitecture, **`agrv2k`** (a Viaduct uarch in `agamemnon/engine/uarch/agrv2k/`): the recovered AGRV2K device graph (50,046 wires, 2,112 LE bels, the routing pips, plus MCU-edge / IO / clock bels) is dumped to flat CSV by `emit_uarch_db.py` and replayed 1:1 into nextpnr, which adds the per-pip conduction gate and placement-legality predicate that stock generic lacks. The uarch is built and loads the full device; its staged bring-up onto silicon is in progress (see `STATUS.md`). Everything silicon-proven to date ran through the bootstrap the uarch replaces — `agamemnon/engine/arch.py`, a pre-pack adapter that emits the *same* device graph so stock `nextpnr-generic` packs, places, and routes combinational, sequential, and MCU-instance designs on it. That bootstrap is what `agamemnon build` uses today. Sequential function is proven on silicon (flip-flops toggle; register-select solved), clock nets route to arbitrary tiles (the open HSE→PLL→global-clock source is emitted directly by bitgen, no vendor clocked baseline), and the MCU↔fabric bridge (`alta_mcu`/`alta_rv3200`) carries both per-bit GPIO (4-bit loopback, 16/16 combos) and the External-AHB memory bus (`*0x60000000` write path). Finally, an open bitstream boots itself from flash: written to the fabric-config region, the boot ROM decompresses and configures it at power-on with no debugger in the loop. Synthesis is stock `yosys` with the AGRV2K LUT4 cell library (`agamemnon/synth/`). See `STATUS.md` for the done/remaining breakdown and `BITSTREAM_FORMAT.md` for the bit-level format.