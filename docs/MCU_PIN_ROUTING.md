...
| Hard UART0 TX, I2C0 SDA/SCL, SPI0 SCK/MOSI/CSN plus IO1 on L48 `PIN_10`–`PIN_17` | Silicon-qualified exact L48 subsets | One open peripheral-route image puts UART0 TX on `PIN_10`, I2C0 SDA/SCL on `PIN_11`/`PIN_15`, and SPI0 SCK/CSN/MOSI on `PIN_12`/`PIN_13`/`PIN_14`. A second exact image adds SPI0 IO1 on `PIN_17`; an active RP2350 PIO slave qualifies 1–4-byte receive values and byte order. These routes are properties of the exact loaded images, not general part pin mappings. I2C additionally needs an external pull-up |
