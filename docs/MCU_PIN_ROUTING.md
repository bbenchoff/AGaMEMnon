...
| PIN_25 combined-cell OE/readback | Silicon-qualified exact subset | Hard-zero data with the recorded six-pip OE corridor qualifies constant release/drive-low, static readback, and local-self-toggle dynamic OE. The ordinary PCF path also qualifies stepped external PIN_10-controlled OE and simultaneous readback under both pulls through the exact `RMUX15 -> RMUX53 -> IMUX11` entry. High-rate simultaneous readback, the divergent RMUX20 branch, generic/open-drain/registered OE, other pins, and other corridors remain unqualified |
...
| UART0 ROM `TX/RX` on `PIN_30/PIN_31` | Application RX silicon-qualified; ROM target protocol pending | The package assignment is documented, and an exact application image qualifies DAP CDC → PIN_31 → GPIO6.1/UART0_UARTRXD at 9600/38400/115200. The mask-ROM protocol and Pico recovery harness still need the five-wire board addition |
| Hard UART0 TX/RX, I2C0 SDA/SCL, SPI0 SCK/MOSI/CSN plus IO1 on L48 `PIN_10`–`PIN_17` and `PIN_31` | Silicon-qualified exact L48 subsets | One open peripheral-route image puts UART0 TX on `PIN_10`, I2C0 SDA/SCL on `PIN_11`/`PIN_15`, and SPI0 SCK/CSN/MOSI on `PIN_12`/`PIN_13`/`PIN_14`. A separate exact image puts UART0 RX on `PIN_31` and receives a 64-byte DAP CDC pattern at three nominal rates. An active RP2350 open-drain slave at `0x55` qualifies I2C bytes plus exact `2A A6`, repeated START, and `5A C3 7E` ACK/ACK/NACK. Another image adds SPI0 IO1 on `PIN_17`; an active RP2350 PIO slave qualifies 1–4-byte receive values and byte order. These routes are properties of the exact loaded images, not general part pin mappings; I2C needs external pull-ups |
