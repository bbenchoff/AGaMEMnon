...
| Fabric outputs `PIN_18` and `PIN_16` | Silicon-qualified | Exactly these two top-edge pads, individually and composed in one image, through the ordinary `agamemnon build <source> --pcf <constraints>`. This is **not** the ten-pad `PIN_10..PIN_19` ring: the other eight top pads have no silicon observation and no entry in `agamemnon/chipdb/pad_output_qualified_L48.csv` |
