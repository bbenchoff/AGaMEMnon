...
| BRAM9K modes and routing | partial | partial | qualified_subset | L48 | One x18 Port-A path, one exact x2 Port-B read/control path, and X13Y4 read-only x9 over 256 aligned addresses with visible data bits 0..2 are qualified. Three x9 INIT projections reconstruct word-address bits 0..8 across the exercised range. The x9 fix is automatic hard-HSE boundary emission plus complete exact masked footprints at two characterized readback route-through sites. Upper x9 data lanes, addresses 256..1023, writes, other modes/sites, output registers, and collisions remain fail-closed. On each port, width candidates still decode byte-exactly only within the documented lowered subset. |
