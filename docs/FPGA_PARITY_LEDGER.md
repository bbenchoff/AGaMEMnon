...
| BRAM9K modes and routing | partial | partial | qualified_subset | L48 | One x18 Port-A path and one exact x2 Port-B read/control path are qualified. On each port, six model-derived width candidates and one invalid control route twice and decode byte-exactly from the fixed-site image; the open emitter accepts only its five lowered direct modes. The recovered 21-hop x9 ingress builds with exact selectors and active readback but remains address-static. Isolated HADDR capture passes, INIT matches the vendor control, independent parity-initialized AddressA[3:5]/IMUX09:07 trials each remained static, and the two reserved non-preamble tail bits were negative as a coupled raw transplant. Those classes are fail-closed; x9/mode-specific clock or read-enable delivery remains unresolved. |
