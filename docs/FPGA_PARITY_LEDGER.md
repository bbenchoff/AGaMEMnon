...
| BRAM9K modes and routing | partial | partial | qualified_subset | L48 | One x18 Port-A path and one exact x2 Port-B read/control path are qualified. On each port, six model-derived width candidates and one invalid control route twice and decode byte-exactly from the fixed-site image; the open emitter accepts only its five lowered direct modes. The recovered x9 ingress, local controls, INIT contents, terminal-identity trials, and complete AddressA route remain functionally negative. Reverse clone descent proves X22Y4 `CFG_IOMUX11[9]` necessary but insufficient; the remaining break is narrowed to a finite default-owned tile-field class. x9 behavior remains unqualified. |
