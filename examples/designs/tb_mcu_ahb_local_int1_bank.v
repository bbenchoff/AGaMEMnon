`timescale 1ns/1ps
module tb_mcu_ahb_local_int1_bank;
  ...
  task write_word(input [1:0] word, input [1:0] value); begin
  ...
  initial begin
    ...
    write_word(2'b11,2'b10);
    if (!irq_pending || irq_mask || irq1) $fatal(1,"masked pending");
    write_word(2'b01,2'b01);
    if (!irq_pending || !irq_mask || !irq1) $fatal(1,"mask enable");
    write_word(2'b11,2'b01);
    if (irq_pending || !irq_mask || irq1) $fatal(1,"acknowledge");
    write_word(2'b11,2'b10);
    if (!irq1) $fatal(1,"re-arm");
    write_word(2'b01,2'b00);
    ...
    $display("PASS: AHB-backed local_int1 pending/mask/ack/re-arm");
    $finish;
  end
endmodule
