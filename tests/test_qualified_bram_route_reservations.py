...
def test_source_reservations_match_final_qualified_trees(tmp_path, profile):
    nets = {name: {'bits': [i], 'attributes': {'keep': 1}}
            for i, name in enumerate(bram.source_signal_routes(profile))}
    ...
    for name, expected in bram.source_signal_routes(profile).items():
...
@pytest.mark.parametrize('profile', sorted(bram.DATA_SOURCE_ROUTES))
@pytest.mark.parametrize('stage', ['prepare', 'canonicalize'])
def test_missing_live_data_tree_is_rejected_atomically(tmp_path, profile, stage):
    # Core control/observer trees alone are insufficient for a write profile.
    nets = {name: {'bits': [i+300]} for i,name in enumerate(bram.expected_routes(profile))}
    source = tmp_path/'missing_data.json'
    source.write_text(json.dumps({'modules': {'top': {'netnames': nets, 'cells': {}}}}))
    before = source.read_bytes()
    with pytest.raises(ValueError, match='lost.*din1'):
        if stage == 'prepare':
            bram.prepare_route_reservations(source, profile)
        else:
            bram.canonicalize_routed_file(source, profile, include_constants=True)
    assert source.read_bytes() == before
...
def test_data_source_placement_rejects_ambiguous_or_changed_source(tmp_path, mutation):
    ...
    nets = {name: {'bits': [i+300]} for i,name in enumerate(bram.source_signal_routes(profile))}
    source = tmp_path/'source.json'
