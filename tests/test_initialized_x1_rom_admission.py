"""Semantic x1 ROM admission: no image, source, content or cell-name allowlist."""
...
def rom_module(name='memory', contents='10010110'):
    return {'attributes': {'AGAMEMNON_CLOCK_SOURCE_PROFILE': 'MCU_BUS_DEFAULT_V1',
            ...
            'cells': {name: {
                ...
                'parameters': {'PORTA_WIDTH': '01111', 'PORTB_WIDTH': '00000',
...
def test_supported_rom_is_not_name_or_content_bound(name, contents):
    state = FEATURE.prepare(rom_module(name, contents), CHIPDB, options_from({}))
    assert state.cells == [(13, 4, 15, 0, 0)]
    assert not state.dual_rw and not state.portb_read


@pytest.mark.parametrize('change', [
    'width18', 'site', 'clock_mode', 'clock_profile', 'clock_class',
...
def test_unqualified_modes_stay_fenced(change):
    module = rom_module()
    ram = module['cells']['memory']
    if change == 'width18': ram['parameters']['PORTA_WIDTH'] = '00000'
    elif change == 'site': ram['attributes']['NEXTPNR_BEL'] = 'X13Y3_BRAM'
...
def test_other_clock_package_contexts_stay_fenced(settings):
    with pytest.raises(SystemExit, match='VP-AGM-006'):
        FEATURE.prepare(rom_module(), CHIPDB, options_from(settings))
