...
files and both routes pack clean. Any drift -- a re-routed JSON whose hash no
longer matches the evidence, or a route that lost a clean selector encoding and
no longer packs -- makes it exit non-zero, and this test fails.
...
If this fails, do NOT edit the evidence hashes by hand. Re-qualify:
  1. re-route serv_rv32i_smoke.v / serv_rv32i_heartbeat.v until they pack
     0-unmapped (needs the toolchain + board),
  2. update rtl.*/hardware.*/verdict by hand from the bench run,
  3. ``python qualification/regen_serv_evidence.py --write`` to sync the hashes.
"""
import subprocess
import sys
