...
@pytest.mark.parametrize('kind,expected', [
    ...
    ('status_overlay_pulse', 'c6db8fd8251f3a4ae7343189c0b7fe231171872704a3216eb8a2b84f2e03f497'),
])
def test_sampling_variant_preserves_all_functional_assertions(kind, expected):
    ...
    original = (Q / ('mcu_ahb_' + kind +
                    ('_test.c' if kind == 'status_overlay_pulse' else '_exact_map_test.c'))).read_text()
    ...
    assert ('if(seen!=0xffu) ++counter_errors;' if kind == 'status_overlay_pulse'
            else 'if(seen!=0xffu) ++counter_coverage_errors;') in variant
