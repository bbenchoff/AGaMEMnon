import json
import os
from pathlib import Path
import shutil
import subprocess

import pytest


ROOT = Path(__file__).resolve().parents[1]


def _iverilog():
    found = shutil.which("iverilog")
    oss = os.environ.get("AGAMEMNON_OSS")
    if not found and oss:
        for name in ("iverilog", "iverilog.exe"):
            candidate = Path(oss) / "bin" / name
            if candidate.is_file():
                return str(candidate)
    return found


def test_id_scratch8_structure_and_evidence():
    source = (ROOT / "qualification" / "mcu_ahb_id_scratch8.v").read_text(
        encoding="utf-8")
    assert ".INIT(16'h8888)" in source
    assert ".INIT(16'hFD5D)" in source
    assert ".INIT(16'hA808)" in source
    records = [
        json.loads(line)
        for line in (ROOT / "qualification" /
                     "mcu_ahb_register_bank_evidence.jsonl").read_text(
                         encoding="utf-8").splitlines()
        if line.strip()
    ]
    record = next(row for row in records
                  if row["trial_id"] ==
                  "2026-08-04-l48-id-scratch8-pure-open")
    assert record["verdict"] == "pass"
    assert record["bitstream_sha256"] == (
        "4cd1551d1202c9768554b75deddcace93291e8444b6d6c82f9762936a7dc737b"
    )


def test_id_scratch8_protocol_simulation(tmp_path):
    compiler = _iverilog()
    if not compiler:
        pytest.skip("Icarus Verilog absent (set AGAMEMNON_OSS or put it on PATH)")
    env = dict(os.environ)
    oss = os.environ.get("AGAMEMNON_OSS")
    if oss:
        env["PATH"] = os.pathsep.join(
            [str(Path(oss) / "bin"), str(Path(oss) / "lib"), env.get("PATH", "")]
        )
    output = tmp_path / "mcu_ahb_id_scratch8.vvp"
    result = subprocess.run([
        compiler, "-g2012", "-s", "tb_mcu_ahb_id_scratch8", "-o", str(output),
        str(ROOT / "qualification" / "mcu_ahb_id_scratch8.v"),
        str(ROOT / "examples" / "designs" / "tb_mcu_ahb_id_scratch8.v"),
    ], env=env, capture_output=True, text=True)
    assert result.returncode == 0, result.stdout + result.stderr
    runtime = Path(compiler).with_name("vvp.exe" if os.name == "nt" else "vvp")
    runner = str(runtime) if runtime.exists() else shutil.which("vvp")
    if not runner:
        pytest.skip("vvp absent")
    run = subprocess.run([runner, str(output)], env=env,
                         capture_output=True, text=True)
    assert run.returncode == 0, run.stdout + run.stderr
    assert "PASS: byte ID/scratch endpoint" in run.stdout
