// Six-bit posted scratch register with direct HWDATA0 storage and one address tag.
module agamemnon_ahb_posted_scratch6_addrtag_core (
  input wire hclk, input wire htrans1, input wire hwrite,
  input wire haddr2, input wire [5:0] hwdata, input wire reset_request,
  output wire hreadyout, output wire hresp, output wire [5:0] hrdata
);
...
`ifdef SYNTHESIS
  ...
  wire scratch5_next;
  wire write_commit_root;
  wire write_commit_lo;
  wire write_commit_hi;
  wire [5:0] scratch;
`else
...
`ifdef SYNTHESIS
  ...
  // The qualified identity route-through footprints form a bounded two-level
  // commit tree.  The root has two consumers and each leaf has three; no
  // branch exceeds the five-consumer boundary demonstrated by scratch5.
  ...
  // Direct HWDATA0 at slice7/I1 is silicon-qualified; removing its separate
  // capture stage frees slice5 for the qualified HWDATA5 consumer.
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'hDD88), .FF_USED(1'b1))
    scratch_storage4(.CLK(hclk),
                     ...
                     .F(), .Q(scratch[4]));
  // Slice5's HWDATA5 input is independently silicon-qualified, but every
  // attempt to deliver a separate dynamic commit input there stayed high.
  // Capture data at that boundary, apply commit/hold in an even combinational
  // slice, and let an ordinary DFF consume the single next-state result.
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'hCACA), .FF_USED(1'b0))
    scratch_next_mux5(.CLK(hclk),
                      .I({1'b0, write_commit_lo, write_data_pipe5, scratch[5]}),
                      .F(scratch5_next), .Q());
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b1))
    scratch_storage5(.CLK(hclk), .I({3'b000, scratch5_next}),
                     .F(), .Q(scratch[5]));
`else
  assign hrdata = haddr2 ? 6'b000000 :
                  {scratch[5:2],
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_posted_scratch6_addrtag_core core_i(
