// Eight-bit posted scratch register. Lane7 folds data, commit, and hold into
// its exact X14Y11 slice0/I1 registered consumer.
module agamemnon_ahb_posted_scratch8_addrtag_core (
  input wire hclk, input wire htrans1, input wire hwrite,
  input wire haddr2, input wire [7:0] hwdata, input wire reset_request,
...
`ifdef SYNTHESIS
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'hCACA), .FF_USED(1'b1))
    scratch_storage7(.CLK(hclk),
                     ...
                     .F(), .Q(scratch[7]));
`else
  assign hrdata = haddr2 ? 8'b00000000 :
                  {scratch[7:2],
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_posted_scratch8_addrtag_core core_i(
