...
module top(input wire clock, input wire reset, output wire led);
  // acc_probe core (2026-09-17): a 32-bit LFSR (x^32+x^22+x^2+x^1, self-seeding) feeds a 16-bit accumulator
  // every cycle; a 12-bit prescaler samples acc[15] into the LED FF every 4096 cycles. Exercises the carry
  // chain (16-bit add), XOR/LUT logic and a wide fanout. LED pattern is deterministic; Fmax is where the
