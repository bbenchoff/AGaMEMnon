// X15Y12 slice2 forwarding-terminal discriminator.
//
// Lane2 storage uses its qualified commit/data/self-feedback footprint.
// The forwarding LUT moves stored data from the failed I1 terminal to I0,
// which the qualified LFSR16 exercised, and places the address tag on I3.
module top;
  ...
  // I0=data, I3=address. 0x00AA = data && !address.
  (* keep, BEL = "X15Y12_SLICE2" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h00AA), .FF_USED(1'b1))
    forwarding(.CLK(hclk), .I({addr_pipe, 2'b00, scratch2}),
               .F(), .Q(hrdata2));
