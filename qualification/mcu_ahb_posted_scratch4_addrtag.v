// Four-bit posted scratch register with one captured write-address tag.
module agamemnon_ahb_posted_scratch4_addrtag_core (
  input wire hclk, input wire htrans1, input wire hwrite,
  input wire haddr2, input wire [3:0] hwdata, input wire reset_request,
  output wire hreadyout, output wire hresp, output wire [3:0] hrdata
);
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_posted_scratch4_addrtag_core core_i(
