#!/usr/bin/env python3
"""Build-audited, opt-in SRAM-only runner for the exact public32 map."""
...
def build_stub():
    out = HERE / "stub_public32_exact_map"
    ...
    subprocess.run([str(TOOL / "riscv64-unknown-elf-gcc.exe"),
                    ...
                    str(HERE / "mcu_ahb_public32_exact_map_test.c")], check=True)
...
def run(image, log_path=None):
    ...
    try:
        ...
        data = [words.get(0x20001000 + 4 * i) for i in range(17)]
        if any(value is None for value in data) or data[16] != 0xc0ffee32:
            raise RuntimeError("incomplete mailbox\n" + log[-4000:])
        passed = (data[0] == 0x000f0002 and data[1:10] == [0] * 9 and
                  data[10] == 0xff and data[11] == 8 and
                  data[12:16] == [0x4147414d, 0, 0, 0])
        print("FCB=%08x errors=%s seen=%02x obs=%u final=%s verdict=%s" %
              (data[0], data[1:10], data[10], data[11],
               ["%08x" % value for value in data[12:16]],
