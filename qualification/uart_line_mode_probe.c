...
static const uint8_t tx_pattern[4] = {0x25u, 0x5au, 0x43u, 0x3cu};
static const uint8_t rx_pattern[4] = {0x7fu, 0x55u, 0x41u, 0x00u};

int main(void) {
    uint32_t fcb = FCB_STAT;
    if (fcb != FCB_STAT_OK)
        fcb = ag32_fcb_config((const uint32_t *)0x20002000u, 99944u / 4u);
    MAILBOX[0] = 0x554d4f44u; /* UMOD */
    ...
    AG32_UART0->CR = 0u;
    AG32_UART0->LCR_H = MAILBOX[14];
    AG32_UART0->ICR = 0x7ffu;
    AG32_UART0->RSR_ECR = 0u;
    AG32_UART0->CR = AG32_UART_CR_RXE | AG32_UART_CR_TXE | AG32_UART_CR_UARTEN;
    ...
    uint32_t error = result ? (uint32_t)result : 0u;
    if (!error && value == 0x7eu) {
        for (unsigned index = 0; index < TRANSFER_BYTES; ++index) {
            result = ag32_uart_putc(AG32_UART0, tx_pattern[index & 3u], 50000000u);
            if (result) { error = 0x100u | (uint32_t)(-result); break; }
            ++transmitted;
            result = ag32_uart_getc(AG32_UART0, &value, 50000000u);
            if (result) { error = 0x200u | (uint32_t)(-result); break; }
            if (value != rx_pattern[index & 3u]) ++mismatch;
            ++received;
        }
    } else if (!error) {
        error = 3u;
    ...
    MAILBOX[13] = 0xc0ffee33u;
    MAILBOX[15] = AG32_UART0->RSR_ECR;
    for (;;) { }
}
