...
from qualification.n58c_hwdata25_i2_hil_s01.classifier import (
...
def test_audit_rejects_i2_contract_tamper_in_disposable_copy(tmp_path: Path) -> None:
    package = tmp_path / "package"
    shutil.copytree(PACKAGE, package, ignore=shutil.ignore_patterns("build", "__pycache__"))
    manifest_path = package / "package_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["route_contract"]["candidate_sink"]["pin"] = "I1"
    ...
    assert completed.returncode != 0
    assert "graph-legal-I2 route contract differs" in completed.stderr
...
def test_manifest_and_preregistration_are_valid_json() -> None:
    ...
    assert manifest["route_contract"]["candidate_sink"]["pin"] == "I2"
