...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0/HWDATA1/HWDATA2 bus-clocked captures and a pure-open three-bit HADDR2-tagged posted-storage footprint are positive; retained discriminators isolate live-HADDR read selection from two stuck-high registered-address branches. The X14Y8 slice8 sparse identity-buffer case remains a constant-one negative; retained HREADYOUT and controller-pulse paths are positive; four apply-to-controller candidate edges are negative. No full-bank, reset, wait/error, lanes 3 through 7, or generic-site claim |
