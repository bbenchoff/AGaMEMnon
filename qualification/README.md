...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 bus-clocked capture and a complete one-bit posted-storage/forwarding footprint are positive; retained HREADYOUT and controller-pulse paths are positive; four apply-to-controller candidate edges are negative. No full-bank, reset, wait/error, or generic-site claim |
