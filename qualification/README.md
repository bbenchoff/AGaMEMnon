...
| `mcu_ahb_bank16_write_isolation_evidence.jsonl` | A deterministic derivative of that held checkpoint qualifies write-commit isolation of +0 against aligned writes to +4/+8/+c through HADDR[3:2]. Reads at all four offsets deliberately alias +0, so read/full address decoding remains open; subword access, upper HRDATA lanes, bursts, arbitrary placement/width and public-bank integration also remain unqualified. |
