...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0/HWDATA1 bus-clocked captures, a complete one-bit footprint, and a two-bit HADDR2-tagged posted-storage footprint are positive; the X14Y8 slice8 sparse identity-buffer case is a retained constant-one negative; retained HREADYOUT and controller-pulse paths are positive; four apply-to-controller candidate edges are negative. No full-bank, reset, wait/error, wider-width, or generic-site claim |
