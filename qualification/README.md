...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank evidence: exact HWDATA0 through HWDATA7 capture/storage footprints and one pure-open combined ID/scratch/counter/W1C bank are positive. The combined image passes ID `0x4d`, all 256 scratch values, ignored ID/counter writes, constant nonzero counter cadence plus phase-swept eight-state coverage, the complete W1C sequence, and cross-register preservation. Its counter uses the qualified seeded X15Y1 carry footprint and W1C events reuse fabric-local low-lane forwarding. The X15Y11 slice12 HRDATA0 branch and earlier direct-control/read variants remain retained coupled negatives, never dead-PIP claims. No integrated-reset, wait/error, byte/halfword, or generic-site claim |
