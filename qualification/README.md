...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank evidence: exact HWDATA0 through HWDATA7 capture/storage footprints and one pure-open combined ID/scratch/counter/W1C bank are positive. The combined image passes ID `0x4d`, all 256 scratch values, ignored ID/counter writes, constant nonzero counter cadence plus phase-swept eight-state coverage, the complete W1C sequence, and cross-register preservation. A second strict image integrates GPIO4.1 synchronous reset, proving asserted-zero scratch/status/counter, blocked writes, two releases/re-arms, and repeated clearing while ID remains immutable. The ledger also records one strict one-hot local-interrupt command bank: HWDATA[3:2] selects causes 16–19, each counted three exact traps with acknowledge/re-arm/masked-hold/reset, and reads remain zero. Its interrupt pending/mask state is shared across the selected lane, not simultaneous per-lane storage. The X15Y11 slice12 HRDATA0 branch and earlier direct-control/read variants remain retained coupled negatives, never dead-PIP claims. No hard-MCU_RESETN, writable-bank wait/error, byte/halfword, or generic-site claim |
