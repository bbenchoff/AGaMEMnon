...
| `mcu_ahb_bank16_read_isolation_evidence.jsonl` | One exact L48 held-scratch derivative qualifies low-16 aligned word reads at +0/+4/+8/+c as `[state,0,0,0]`, with four decoder controls and four repeated real-decoder runs. Existing word/halfword/byte writes, retention, overwrite, wait and reset behavior remain intact. Subword read-lane semantics, HRDATA[31:16], higher/full-window decode, bursts, arbitrary placement/width, a generic generator and public-bank integration remain open. |
