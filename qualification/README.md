...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA6 bus-clocked capture/storage footprints and a pure-open seven-bit HADDR2-tagged posted-storage footprint are positive. Lane5 uses its exact slice5 capture, a separate commit/hold next-state LUT, and one-input slice8 storage behind a two-leaf characterized route-through distribution tree. Lane6 folds HWDATA6, commit, and own-Q hold into its exact X14Y12 slice15 consumer while HREADYOUT uses a separate strict constant source. Earlier direct-control variants remain retained negatives. No full bank, reset, wait/error, lane7 storage, or generic-site claim |
