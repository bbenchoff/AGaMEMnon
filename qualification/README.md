...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA5 bus-clocked captures and a pure-open five-bit HADDR2-tagged posted-storage footprint are positive; direct HWDATA0 at lane-zero storage and all four X14Y11 slice5 HWDATA5 terminals are live. The slice5 DD88 storage/data/own-Q footprint also follows HWDATA5 with commit tied high. All four X15Y12 slice4 HWDATA5 candidates are constant-one negatives; routed-control six-bit variants preserve exact lower-five behavior while lane5 stays constant one, localizing but not closing control delivery. No full-bank, reset, wait/error, lanes 5 through 7 storage, or generic-site claim |
