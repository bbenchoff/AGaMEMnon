...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA4 bus-clocked captures and a pure-open five-bit HADDR2-tagged posted-storage footprint are positive; all four X15Y12 slice2 HWDATA4 terminals are live, while all four X14Y12 slice2 candidates are constant-one negatives. Retained discriminators also isolate live-HADDR read selection from two stuck-high registered-address branches and an intentionally ungated lane-three read. The X14Y8 slice8 sparse identity-buffer case remains a constant-one negative; retained HREADYOUT and controller-pulse paths are positive; four apply-to-controller candidate edges are negative. No full-bank, reset, wait/error, lanes 5 through 7 storage, or generic-site claim |
