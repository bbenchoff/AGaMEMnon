...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank evidence: exact HWDATA0 through HWDATA7 capture/storage footprints and one pure-open combined ID/scratch/counter/W1C bank are positive. The combined image passes ID `0x4d`, all 256 scratch values, ignored ID/counter writes, constant nonzero counter cadence plus phase-swept eight-state coverage, the complete W1C sequence, and cross-register preservation. A second strict image integrates GPIO4.1 synchronous reset. A waited variant qualifies scratch lanes 0–5 and 7 across all 256 values and 128 back-to-back pairs; lane6 is explicitly tied zero, so this is not a full-byte claim. Real aligned halfword/word transfers preserve that projection. Strict follow-ons drive HRDATA[15:8] zero while preserving the relocated HRDATA7 exit; HRDATA12 and HRDATA15 reuse the qualified registered-zero scratch6 source, while HRDATA13–14 use free exact GND ingresses. Aligned halfword reads return exact zero-extended `value & 0xbf` results across all 256 cases and 128 mixed pairs. HRDATA[31:16] remains undriven, so exact word reads remain unsupported. The ledger also records one strict one-hot local-interrupt command bank: HWDATA[3:2] selects causes 16–19, each counted three exact traps with acknowledge/re-arm/masked-hold/reset, and reads remain zero. Its pending/mask state is shared across the selected lane. The attached-board post-reset/pre-SRAM-config local MIP boundary is zero with local MIE clear or armed; configured reset/release is zero; 64 set and 64 ack transitions each take exactly 21 default-clock MTIME ticks and synchronous reset clear takes 40. These are not POR, blank-fabric, PLL3, alternate-clock, or hard-MCU_RESETN claims. The X15Y11 slice12 HRDATA0 branch, lane6 wait compositions, local-feedback controller variants, early-high-commit phase, lane6-only commit-stage-F retry, two-wait response extension, own-Q pin-swap retry, and separate-storage retry remain retained coupled negatives, never dead-PIP claims. A raw-Q HRDATA8 witness matches the ordinary HRDATA6 branch across all 256 cases and localizes its sticky-high failure to stored state; replacement storage has no effect. Restoring the qualified HWDATA6 ingress clears the basic signature but leaves a one-transfer lag under exhaustive/pair coverage. No full-byte writable wait/error, complete upper-zero word, byte, or generic-site claim |
