...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA5 bus-clocked captures and a pure-open six-bit HADDR2-tagged posted-storage footprint are positive. Lane5 uses its exact slice5 capture, a separate commit/hold next-state LUT, and one-input slice8 storage behind a two-leaf characterized route-through distribution tree. Earlier direct-control variants remain retained negatives. No full-bank, reset, wait/error, lanes6..7 storage, or generic-site claim |
