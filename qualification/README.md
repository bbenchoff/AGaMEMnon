...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA5 bus-clocked captures and a pure-open five-bit HADDR2-tagged posted-storage footprint are positive; direct HWDATA0 at lane-zero storage and all four X14Y11 slice5 HWDATA5 terminals are live. All four X15Y12 slice4 HWDATA5 candidates are constant-one negatives. Three coupled six-bit trials preserve exact lower-five behavior while lane5 stays constant one with commit on I0/I1 and direct readback, so they narrow but do not qualify the storage-mode footprint. Retained discriminators also isolate live-HADDR read selection from stuck-high registered-address branches and an intentionally ungated lane-three read. No full-bank, reset, wait/error, lanes 5 through 7 storage, or generic-site claim |
