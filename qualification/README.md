...
| `mcu_slave_ahb_request_payload_route_evidence.jsonl` | Vendor and strict-open evidence for all 64 fabric-master request payload lanes from one dual-output safe-low source, plus one exact registered HADDR[2] route with the other 63 lanes held safe-low. The latter has 40/40 retained selector-bit agreement and a strict-open route replay; no arbitrary payload placement, transaction, or silicon claim. |
...
| `bram_site_read_evidence.jsonl` | fresh-source X13Y3/X13Y4 x18 full-depth opt-in passes, exact corpus/table hashes, causal HSE and identity-buffer negatives, and the retained partial X13Y1/X13Y2 boundary |
