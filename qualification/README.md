...
| `mcu_ahb_register_bank_evidence.jsonl` | L48 bounded register-bank pipeline evidence: exact HWDATA0 through HWDATA7 bus-clocked capture/storage footprints and a pure-open complete-byte HADDR2-tagged posted store are positive over all 256 values. Lane5 uses its exact slice5 capture, a separate commit/hold next-state LUT, and one-input slice8 storage behind a two-leaf characterized route-through distribution tree. Lanes6 and 7 fold data, commit, and own-Q hold into their exact consumers while HREADYOUT uses a separate strict constant source. Earlier direct-control variants remain retained negatives. No full register-class bank, integrated reset, wait/error, or generic-site claim |
