// Reference RTL shape for the silicon-qualified write-isolated held 16-bit
// External-AHB scratch. The retained routed checkpoint is produced by
// compose_mcu_ahb_bank16_word0_write_isolation.py from the already-qualified
// one-wait external-feedback route. This source documents the equivalent logic;
// it is not claimed to reproduce that placement through unconstrained routing.
// The design uses one controller and one external combinational feedback buffer
// per state bit.
//
...
// would ask OMUX20 to carry two different nets. The retained composition keeps
// the wait controller there and preserves capture12 at X14Y10_SLICE6 from the
// qualified held-bank route; this explanatory source leaves capture12 unbound.
//
...
(* top *)
module top;
  wire hclk, htrans1, hwrite, haddr2, haddr3, reset_request;
  wire write_pending, write_ready_f, base_enable, hresp;
  ...
  // Address-phase decode for the sixteen-byte aperture. Only +0 accepts a
  // write; +4, +8 and +c are bounded foreign offsets. HADDR[1:0] are ignored
  // at this stage and subword semantics remain intentionally unclaimed.
  (* keep, BEL = "X14Y12_SLICE0" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h0011), .FF_USED(1'b0))
    addr_word0_decode(.CLK(hclk),
                      .I({haddr3, 1'b0, reset_request, haddr2}),
                      .F(base_enable), .Q());

  (* keep, BEL = "X14Y12_SLICE1" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h8000), .FF_USED(1'b1))
    write_stage(.CLK(hclk),
                .I({base_enable, write_ready_f, hwrite, htrans1}),
                .F(), .Q(write_pending));
