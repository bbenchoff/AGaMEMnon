// Silicon-qualification image for the combined low-byte register classes:
//   0x0 immutable ID 0x4d, 0x4 posted scratch, 0x8 counter[2:0],
//   0xc one-bit W1C status (write bit1=set hook, bit0=clear).
module agamemnon_ahb_register_bank_combined_core (
...
`endif
  // --hard-carry lowers this reset-free adder into the already-qualified
  // seed + three-stage X15Y1 dedicated-carry footprint.
  always @(posedge hclk)
...
`ifdef SYNTHESIS
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'hDD88), .FF_USED(1'b1))
    scratch_storage1(.CLK(hclk),
                     .I({scratch[1], 1'b0, write_data_pipe1,
  ...
  always @(posedge hclk) begin
    if (reset_request) begin
      ...
      if (scratch_commit_root)
        scratch <= {hwdata[7:6], write_data_pipe5,
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_register_bank_combined_core core_i(
    .hclk(hclk), .htrans1(htrans1), .hwrite(hwrite),
    .haddr2(haddr2), .haddr3(haddr3), .hwdata(hwdata),
    .reset_request(1'b0), .hreadyout(hreadyout), .hresp(hresp),
    .hrdata(readback));
endmodule
