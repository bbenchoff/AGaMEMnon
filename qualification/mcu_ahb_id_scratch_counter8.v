// Byte-wide ID/scratch/counter endpoint. Offset 8 exposes an independent
// free-running read-only counter; offset C remains zero for the W1C unit.
module agamemnon_ahb_id_scratch_counter8_core (
...
`ifdef SYNTHESIS
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux2(.CLK(hclk), .I({low_read[2], haddr3, haddr2, counter[2]}), .F(hrdata[2]), .Q());
  (* keep, BEL = "X15Y11_SLICE11" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux3(.CLK(hclk), .I({low_read[3], haddr3, haddr2, counter[3]}), .F(hrdata[3]), .Q());
  (* keep, BEL = "X15Y11_SLICE12" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux4(.CLK(hclk), .I({low_read[4], haddr3, haddr2, counter[4]}), .F(hrdata[4]), .Q());
  (* keep, BEL = "X15Y11_SLICE13" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux5(.CLK(hclk), .I({low_read[5], haddr3, haddr2, counter[5]}), .F(hrdata[5]), .Q());
  (* keep, BEL = "X15Y11_SLICE14" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux6(.CLK(hclk), .I({low_read[6], haddr3, haddr2, counter[6]}), .F(hrdata[6]), .Q());
  (* keep, BEL = "X15Y11_SLICE15" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h2F20), .FF_USED(1'b0))
    class_mux7(.CLK(hclk), .I({low_read[7], haddr3, haddr2, counter[7]}), .F(hrdata[7]), .Q());
...
`else
  ...
  assign hrdata = haddr3 ? (haddr2 ? 8'h00 : counter) : low_read_sim;
`endif
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_id_scratch_counter8_core core_i(
