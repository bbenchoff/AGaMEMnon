...
module top(input wire clock, input wire reset, output wire led);
  // acc_probe core (2026-09-17): a 32-bit LFSR (x^32+x^22+x^2+x^1, self-seeding) feeds a 32-bit accumulator
  // every cycle; a 12-bit prescaler samples acc[31] into the LED FF every 4096 cycles. Exercises the carry
  // chain (32-bit add), XOR/LUT logic and a wide fanout. LED pattern is deterministic; Fmax is where the
  ...
  always @(posedge clock) begin
      ...
      // fuses a register into a carry slice when D = SUM); reset instead zeroes the addend. Sequence identical.
      acc <= acc + (rst_r ? 32'd0 : lfsr);
      if (rst_r) begin lfsr <= 32'd0; pre <= 12'd0; led_r <= 1'b0; end
      else begin
          lfsr <= (lfsr == 32'd0) ? 32'hDEADBEEF : {lfsr[30:0], fb};
          pre  <= pre + 12'd1;
          if (pre == 12'hFFF) led_r <= acc[31];
      end
  end
  (* keep *) wire led_i;
