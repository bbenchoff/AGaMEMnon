// Four-site counter with the three-input carry isolated in a combinational LUT.
module top;
  wire bus_clock;
  wire q0, q1, q2, q3;
  wire f0, f1, f2, f3;
  ...
  // carry3 = q2 & q1 & q0 = ~f2 & ~f1 & ~f0
  (* keep, BEL="X14Y10_SLICE0" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h0001), .FF_USED(1'b0))
    carry_lut(.I({1'b0, f2, f1, f0}), .F(carry3));

  (* keep, BEL="X14Y11_SLICE7", agamemnon_direct_d_feedback="1" *)
  GENERIC_SLICE #(.K(4), .INIT(16'h55aa), .FF_USED(1'b1))
    bit3(.CLK(bus_clock), .I({q3, 2'b00, carry3}), .F(f3), .Q(q3));
