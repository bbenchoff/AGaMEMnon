// Silicon-qualification image for the GPIO4.1-resettable combined bank:
//   0x0 immutable ID 0x4d, 0x4 posted scratch, 0x8 counter[2:0],
//   0xc one-bit W1C status (write bit1=set hook, bit0=clear).
module agamemnon_ahb_register_bank_combined_gpio_reset_core (
...
`ifdef SYNTHESIS
  ...
  GENERIC_SLICE #(.K(4), .INIT(16'h00B8), .FF_USED(1'b1))
    scratch_storage6(.CLK(hclk),
                     .I({reset_request, scratch[6], write_commit_hi, hwdata[6]}),
                     .F(), .Q(scratch[6]));
...
module top;
  ...
  (* keep *) GENERIC_SLICE #(.K(4), .INIT(16'hAAAA), .FF_USED(1'b0))
    readback_buffer0(.CLK(hclk), .I({3'b000, readback[0]}),
                     .F(hrdata0), .Q());
  agamemnon_ahb_register_bank_combined_gpio_reset_core core_i(
