...
from qualification.n58d_hwdata25_i3_hil_s01.classifier import (
...
def test_audit_rejects_i3_contract_tamper_in_disposable_copy(tmp_path: Path) -> None:
    package = tmp_path / "package"
    shutil.copytree(PACKAGE, package, ignore=shutil.ignore_patterns("build", "__pycache__"))
    ...
    assert completed.returncode != 0
    assert "graph-legal-I3 route contract differs" in completed.stderr
...
def test_manifest_and_preregistration_are_valid_json() -> None:
    ...
    assert manifest["route_contract"]["candidate_sink"]["pin"] == "I3"
