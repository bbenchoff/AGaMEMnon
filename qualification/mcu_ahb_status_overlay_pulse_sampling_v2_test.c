// SRAM-only causal oracle for the generic status-overlay path.
// The same firmware runs against the unchanged core, a routed zero-source
// control, and the ordinary-Verilog pulse overlay.
typedef unsigned int u32;
typedef unsigned short u16;
typedef unsigned char u8;
#define R(a) (*(volatile u32 *)(a))
#define GPIO4 0x40018000u
#define GPIO_DATA(base, mask) R((base) + ((mask) << 2))
#define GPIO_DIR(base) R((base) + 0x400u)
#define GPIO_AFSEL(base) R((base) + 0x420u)

static inline __attribute__((always_inline)) void fence_bus(void) {
  __asm__ volatile("fence iorw, iorw" ::: "memory");
}
static inline __attribute__((always_inline)) void settle(void) {
  for (volatile u32 i=0;i<256u;++i) __asm__ volatile("nop");
}
static inline __attribute__((always_inline)) void gpio_reset(u32 asserted) {
  GPIO_DATA(GPIO4, 1u << 1) = asserted ? (1u << 1) : 0u;
  fence_bus();
}

void __attribute__((section(".text.start"))) _start(void) {
  volatile u32 *out=(volatile u32 *)0x20001000u;
  volatile u32 *cfg=(volatile u32 *)0x20002000u;
  volatile u32 *word=(volatile u32 *)0x60000000u;
  volatile u16 *half=(volatile u16 *)0x60000000u;
  volatile u8 *byte=(volatile u8 *)0x60000000u;
  u32 raw_errors=0, lane_errors=0, scratch_errors=0;
  u32 counter_errors=0, status_signature=0, foreign_errors=0;
  u32 reset_errors=0, zeroext_errors=0, seen=0, observations=0;

  R(0x0300000cu)&=~0x27u;
  R(0x03000060u)|=(1u<<0)|(1u<<8);
  R(0x03000030u)=0;
  GPIO_AFSEL(GPIO4)=0u;
  GPIO_DIR(GPIO4)=(1u<<1);

  // Deassert before activation so the ordinary user pulse cannot be hidden by
  // the core reset.  The pulse is emitted once, immediately after FCB accepts
  // the volatile image; the stored W1C state makes it observable later.
  gpio_reset(0);
  R(0x40010000u)=(1u<<6);
  for(u32 i=0;i<24986u;++i) R(0x4001000cu)=cfg[i];
  settle();
  u32 stat=R(0x40010010u);

  status_signature |= (word[3]&1u) << 0;
  word[3]=1u; fence_bus(); settle();
  status_signature |= (word[3]&1u) << 1;

  // Only the unchanged negative retains the qualification-only bit1 hook.
  word[3]=2u; fence_bus(); settle();
  status_signature |= (word[3]&1u) << 2;
  word[3]=1u; fence_bus(); settle();
  status_signature |= (word[3]&1u) << 3;

  if(word[0]!=0x4147414du) ++raw_errors;
  if(byte[0]!=0x4du || byte[1]!=0x41u || byte[2]!=0x47u ||
     byte[3]!=0x41u || half[0]!=0x414du || half[1]!=0x4147u)
    ++lane_errors;
  if(((u32)byte[0]&0xffffff00u) || ((u32)half[0]&0xffff0000u))
    ++zeroext_errors;

  // Eight independent public-map observations keep this oracle honest without
  // duplicating the already-qualified scratch write matrix.
  for(u32 i=0;i<8u;++i) {
    if(word[0]!=0x4147414du || word[1]!=0u || word[3]!=0u)
      ++scratch_errors;
    ++observations;
  }
  u32 sampling_state=0x1234567u;
  for(u32 i=0;i<2048u;++i) {
    u32 c=word[2];
    if(c&~7u) ++counter_errors;
    seen|=1u<<(c&7u);
    if(word[0]!=0x4147414du || word[1]!=0u || word[3]!=0u)
      ++foreign_errors;
    sampling_state^=sampling_state<<13;
    sampling_state^=sampling_state>>17;
    sampling_state^=sampling_state<<5;
    for(volatile u32 j=0;j<(sampling_state&31u);++j) __asm__ volatile("nop");
  }
  if(seen!=0xffu) ++counter_errors;

  // Core reset clears every mutable word.  Releasing it cannot re-arm this
  // user fragment: it has no reset input and its one-shot already completed.
  gpio_reset(1); settle();
  if(word[0]!=0x4147414du || word[1]!=0u || word[2]!=0u || word[3]!=0u)
    ++reset_errors;
  word[1]=0xffffu; word[3]=2u; fence_bus(); settle();
  if(word[1]!=0u || word[3]!=0u) ++reset_errors;
  gpio_reset(0); settle();
  status_signature |= (word[3]&1u) << 4;

  gpio_reset(1); settle();
  out[0]=stat; out[1]=raw_errors; out[2]=lane_errors;
  out[3]=scratch_errors; out[4]=counter_errors; out[5]=0;
  out[6]=status_signature; out[7]=foreign_errors; out[8]=reset_errors;
  out[9]=zeroext_errors; out[10]=seen; out[11]=observations;
  out[12]=word[0]; out[13]=word[1]; out[14]=word[2]; out[15]=word[3];
  out[16]=0xc0ffee32u;
  for(;;) __asm__ volatile("ebreak");
}
