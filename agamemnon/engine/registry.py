...
OPTIONS = {
    ...
    "AGAMEMNON_FROM_SCRATCH_BASE": _flag("bitgen", "experimental", "docs/FABRIC_DEFAULT_CANVAS.md", "Experimental: synthesize the design-neutral base image from scratch (preamble + named border config + col-58 framing + reserved routing/seam SRAM reset fill from the promoted logictile_config_template) via default_frame instead of loading fabric_default.bin; reconstructs ~99.77% of the canvas body byte-exact, with ~227 region-edge partial bytes still left at zero pending a per-bit decode."),
