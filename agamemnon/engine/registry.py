...
OPTIONS = {
    ...
    "AGAMEMNON_MCU_GPIO4_REQUEST_PATHS": _flag("both", "experimental", "agamemnon/chipdb/mcu_gpio4_request_paths.csv", "Expose the measured GPIO4 bit-1 (AHB 'request') MCU-output->fabric-input entry corridor: the X11Y5_BufMUX10 boundary plus the one novel X11Y4_RMUX81 fabric bridge. Opt-in so the default device graph is byte-unchanged (zero blast radius); the corridor is measured (vendor route + codewords triangulated with four ahbrwide corpus builds) but silicon-unproven until an attended board witness clears the open BRAM-collision handshake."),
