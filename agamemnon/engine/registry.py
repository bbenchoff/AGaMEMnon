...
OPTIONS = {
    ...
    "AGAMEMNON_BRAM_NARROW_WRITE": _flag("both", "experimental", "qualification/bram_narrow_write_evidence.jsonl", "Enable native narrow-width (x9/x4/x2/x1) Port-A BRAM writes by replicating the logical DataInA across every address-selected physical write window. The vendor alta_bram9k write mask is address-selected, so the packer's default lowest-window-only DataIn drop makes non-lowest addresses silently keep their old value; qin_pack.replicate_narrow_bram_write_datain fans the data onto all windows, the agrv2k packer keeps those real-driven padded lanes (constant/dangling padded lanes still trim, so SERV x2 stays byte-identical), and the emitter's self-verifying guard admits a width only when every window is populated. Fix is iverilog-verified against the vendor model (errors=0 all widths); each width still requires board qualification. Opt-in so default builds keep the fail-closed P0 refusal and are byte-identical."),
