...
def main(argv=None, environ=None):
    ...
    # Complete, silicon-observed identity route-through footprints.  These are
    # intentionally exact-site/exact-final-edge entries: a LUT INIT overlay is
    # not sufficient because the vendor configures the physical LUT-input
    # permutation and its IMUX selector as one coherent slice footprint.
    _rt_footprints = load_footprints(SRCA + "/route_through_footprints.csv")
    _routed_nets = [(_n, set(_v.get("bits", [])), _v.get("attributes", {}).get("ROUTING", ""))
                    for _n, _v in mod.get("netnames", {}).items()]
    ...
    for cn, c in mod["cells"].items():
        ...
        slices.append((x, y, z))
        try:
            route_through_writes.extend(
                complete_footprint_for_cell(c, _routed_nets, _rt_footprints)
            )
    ...
    if route_through_writes:
        print("complete route-through footprint bytes: %d" % len(route_through_writes))
