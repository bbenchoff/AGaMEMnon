...
static inline void status_source(uint32_t asserted) {
    GPIO5_DATA(1u << 0) = asserted ? (1u << 0) : 0u;
    fabric_fence();
}
...
int main(void) {
    ...
    SYSCTL_APBCLK |= APBCLK_GPIO4 | APBCLK_GPIO5;
    GPIO4_AFSEL &= ~(1u << 1);
    GPIO4_DIR |= 1u << 1;
    GPIO5_AFSEL &= ~(1u << 0);
    status_source(0u);
    ...
    // The exact GPIO5 profile has no bit1 software-set hook. A sustained MCU
    // GPIO5 DATA0 level sets; dropping it retains; W1C bit0 then clears.
    status_source(1u);
    settle();
    result[7] = word[3] & 1u;
    status_source(0u);
